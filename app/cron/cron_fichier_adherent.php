...
foreach ($p_ffcam as $ffcam) {
    $fileTab[] = dirname(dirname(dirname(__FILE__))).DS.'config'.DS.MON_DOMAINE.DS.'ffcam'.DS.$ffcam.'.txt';
}
...
foreach($fileTab as $file){
	...
	if(file_exists($file) && is_file($file)){
		...
		$handle = @fopen($file, "r") or die ("ouverture du fichier '$file' impossible : $!");
		echo  "tous les droits de traitement\n";
		if($handle){
			echo  "handle\n";
			$i=0;
			while(($line=fgets($handle)) !== false){
				...
				if(sizeof($line) < $p_csv_adherent_nb_colonnes) {
					$messg = "Format invalide : la ligne ne contient pas $p_csv_adherent_nb_colonnes valeurs mais ".sizeof($line)."\n";
					echo $messg;
					$tmpErrTab[]=$messg;
				}
				else{echo $i."\nligne ".__LINE__."\n";
					if(!is_numeric($line[0])) 	$tmpErrTab[]="Numéro d'adhérent complet invalide"; // numéro d'adhérent
					if(!is_numeric($line[1])) 	$tmpErrTab[]="Numéro de club invalide";echo $i."\nligne ".__LINE__."\n";
					if(!is_numeric($line[2])) 	$tmpErrTab[]="Numéro d'adhérent invalide";echo $i."\nligne ".__LINE__."\n";
					if(!preg_match("#[0-9]{4}-[0-9]{2}-[0-9]{2}#", $line[6])) 	$tmpErrTab[]="Date de naissance invalide";
					// STOP : inutile de trop brider, laissons une marge d'erreur, corrigeable dans la base du site
				}

				// si erreur, affichée dans le log
				if(sizeof($tmpErrTab) > 0){
					print "!!! Erreurs ligne $i :\n !!! - ".implode('\n -- ', $tmpErrTab)."\n";
					print_r ($line);
				...
				// lecture des données
					...
					echo 'formatage des variables';echo $i."\nligne ".__LINE__."\n";
...
//echo $i."\nligne ".__LINE__."\n";
					...
					$formatTab = array(8, 9, 10); // index dans le tableau, des elements à formater
...
//echo $i."\nligne ".__LINE__."\n";
				...
				//echo  $req."\n";		
					...
					if ( mysqli_num_rows($handleSql) == 0) {
						print "aucun ID trouve pour $cafnum_user";
		...
		print "INSERT: $nb_insert, UPDATE:$nb_update\n";

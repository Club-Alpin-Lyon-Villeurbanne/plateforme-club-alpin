...
function cron_email($datas){
	...
	$tmpErr='';
	if(!$datas['parent']) $tmpErr.=" parent = ".html_utf8($datas['parent'])."";
	if(!$datas['code']) $tmpErr.=" code : ".html_utf8($datas['code'])."";
	if(!isMail($datas['email'])) $tmpErr.=" E-mail invalide : ".html_utf8($datas['email'])."";
	if(!$datas['name']) $tmpErr.=" name : ".html_utf8($datas['name'])."";
	if(!$datas['subject']) $tmpErr.=" subject : ".html_utf8($datas['subject'])."";
	if(!$datas['content']) $tmpErr.=" content : ".html_utf8($datas['content'])."";
	if($tmpErr && admin()) echo '>>>>>>>>>>>>> ERROR <<<<<<<<<<<< '.$tmpErr;
...
if($lastTsp < $minTsp){
	...
	if($chron_savedatas){
		if(!$mysqli->query("INSERT INTO ".$pbd."chron_launch(id_chron_launch, tsp_chron_launch) VALUES('', '$p_time');")) $errTab[]="Erreur d'enregistrement du launch";
		$id_chron_launch = $mysqli->insert_id;
	}
	else $id_chron_launch=1; // Developpement : fixe un parent fictif a un envoi de mail par exemple
	...
	if(!sizeof($errTab)){
		...
		error_log(''.$nEltsToValidate.' evenements a valider, '.sizeof($datasTab).' responsables a contacter.');
		...
		foreach($datasTab as $email=>$evtdatas){
			...
			$ids_evt='';
			foreach($evtdatas as $tmp) $ids_evt.=($ids_evt?'-':'').$tmp['id_evt'];
...
//			foreach($evtdatas as $tmp) $evtList[] = '<a href="'.$p_racine.'gestion-des-sorties.html">'.date('d/m/Y', $tmp['tsp_evt']).' - '.html_utf8($tmp['titre_evt']).'</a>';
			...
			$datas['subject']=sizeof($evtdatas)." sorties à valider";
			$datas['content']='
				<h1>Bonjour '.html_utf8($evtdatas[0]['firstname_user']).',</h1>
				<p>
				Vous êtes responsable de commission, et il y a '.sizeof($evtdatas).' sortie(s) en attente de publication sur le site du '.$p_sitename.'.
		...
		foreach($datasTab as $email=>$evtdatas){
			...
			$datas['email']=$email;
			$datas['name']=html_utf8($evtdatas[0]['civ_user'].' '.$evtdatas[0]['firstname_user'].' '.$evtdatas[0]['lastname_user']);
			$datas['subject']=sizeof($evtdatas)." sorties a valider";
			$datas['content']='
				<h1>Bonjour '.html_utf8($evtdatas[0]['firstname_user']).',</h1>
				<p>
				Vous êtes en charge de la validation légale des sorties, et il y a  '.sizeof($evtdatas).' sortie(s) en attente de validation sur le site du '.$p_sitename.'.
...
if(sizeof($errTab)){
	if(admin()){
		foreach($errTab as $err) error_log ("$err");
	}
}

<?php
    ...
    if ($app_version < '1.0.1') {
        ...
        $queries[] = 
            'CREATE TABLE IF NOT EXISTS `'.$pbd.'user_niveau` (
        ...
        $queries[] = 
            "INSERT INTO ".$pbd."`userright` (`id_userright`, `code_userright`, `title_userright`, `ordre_userright`, `parent_userright`) 
            VALUES 
    ...
    if ($app_version < '1.1') {
        ...
        // Feuille de sortie
        $queries[] =
            "INSERT INTO `".$pbd."page` VALUES(NULL, 0, 0, 0, 1, 0, 0, 0, 0, 0, 'feuille-de-sortie', 'Feuille de sortie', 0, 0, '0.0', '', '', 0, 0, 0);";

        /* FICHE SORTIE */
        // Gestion des distances
        $queries[] = 
            'ALTER TABLE `'.$pbd.'evt` CHANGE `distance_evt` `distance_evt` FLOAT(10,2) UNSIGNED NULL DEFAULT NULL;';
        /* Itinéraire : descriptif textuel */
        $queries[] = 
            'ALTER TABLE `'.$pbd.'evt` ADD `itineraire` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL AFTER `difficulte_evt`;';
        /* Dénivelé : chiffre en (m) */
        $queries[] = 
            'ALTER TABLE `'.$pbd.'evt` CHANGE `denivele_evt` `denivele_evt` INT(5) UNSIGNED NULL DEFAULT NULL;';
        /* Détail des tarifs */
        $queries[] = 
            'ALTER TABLE `'.$pbd.'evt` ADD `tarif_detail` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL AFTER `tarif_evt`;';
        /* Tarif facultatif de repas au restaurant */
        $queries[] = 
            "ALTER TABLE `".$pbd."evt` ADD `tarif_restaurant` FLOAT(10,2) UNSIGNED NULL DEFAULT NULL AFTER `tarif_detail`;";
        /* Repas facultatif au restaurant */
        $queries[] = 
            "ALTER TABLE `".$pbd."evt` ADD `repas_restaurant` TINYINT(1) UNSIGNED NOT NULL DEFAULT '0' AFTER `tarif_detail`;";
        /* Tarif NULL */
        $queries[] = 
            "ALTER TABLE `".$pbd."evt` CHANGE `tarif_evt` `tarif_evt` FLOAT(10,2) NULL DEFAULT NULL;";
        /* Groupe */
        $queries[] = 
            "ALTER TABLE `".$pbd."evt` ADD `id_groupe` INT(11) UNSIGNED NULL DEFAULT NULL AFTER `commission_evt`;";
            
        /* GROUPES DE NIVEAUX */
        // Gestion des groupes
        $queries[] =
            "CREATE TABLE IF NOT EXISTS `".$pbd."groupe` (
              `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
              `id_commission` int(11) unsigned NOT NULL,
        ...
        // Gestion des lieux
        $queries[] = 
            "CREATE TABLE IF NOT EXISTS `".$pbd."lieu` (
              `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
        ...
        $queries[] = 
            "CREATE TABLE IF NOT EXISTS `".$pbd."destination` (
        ...
        $queries[] =
            "INSERT INTO `".$pbd."page` (`id_page`, `parent_page`, `admin_page`, `superadmin_page`, `vis_page`, `ordre_page`, `menu_page`, `ordre_menu_page`, `menuadmin_page`, `ordre_menuadmin_page`, `code_page`, `default_name_page`, `meta_title_page`, `meta_description_page`, `priority_page`, `add_css_page`, `add_js_page`, `lock_page`, `pagelibre_page`, `created_page`)
        ...
        // Gestion des Bus 
        $queries[] = 
            "CREATE TABLE IF NOT EXISTS `".$pbd."bus` (
              `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
        ...
        $queries[] = 
            "CREATE TABLE IF NOT EXISTS `".$pbd."bus_lieu_destination` (
              `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
        ...
        $queries[] = 
            "ALTER TABLE `".$pbd."evt_join` ADD `id_bus_lieu_destination` INT(11) UNSIGNED NULL DEFAULT NULL ;";
        $queries[] =
            "ALTER TABLE `".$pbd."evt_join` ADD `id_destination` INT(11) UNSIGNED NULL DEFAULT NULL , ADD `is_covoiturage` TINYINT(1) UNSIGNED NULL DEFAULT NULL ;";
        // Utilisateur choisit d'aller au restaurant (facultatif : null (non proposé, 0, 1)
        $queries[] =
            "ALTER TABLE `".$pbd."evt_join` ADD `is_restaurant` TINYINT(1) UNSIGNED NULL DEFAULT NULL ;";
        // table d'association sortie / destination
        $queries[] =
            "CREATE TABLE IF NOT EXISTS `".$pbd."evt_destination` (
              `id` int(11) unsigned NOT NULL AUTO_INCREMENT,

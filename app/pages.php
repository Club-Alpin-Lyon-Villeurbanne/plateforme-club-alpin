...
while($handle = $handleSql->fetch_array(MYSQLI_ASSOC)) {
	$p_pages[$handle['code_page']] = array(
...
if ($p_pages[$p2])		$codePrioritaire = $p2;
elseif ($p_pages[$p1])	$codePrioritaire = $p1;

// Les pages d'accueil commission sont prioritaires
if ($p1 == 'accueil')	$codePrioritaire = $p1;
if ($p1 == 'creer-une-sortie')	$codePrioritaire = $p1;
...
if($p_pages[$codePrioritaire]){
	...
	$meta_title=$p_pages[$codePrioritaire]['meta_title_page']?$p_pages[$codePrioritaire]['meta_title_page']:cont('site-meta-title');
	$meta_description=$p_pages[$codePrioritaire]['meta_description_page']?$p_pages[$codePrioritaire]['meta_description_page']:cont('site-meta-description');
...
// PAS TROUVE
	...
	header ('Location: '.$p_racine.($p_multilangue?$lang.'/':'').'accueil.html');
	exit;
}
...
if($mode=='edit' && !$id_evt) $errTab[]="ID sortie manquant";

if(!sizeof($errTab)){
	// modification de sortie
	if($mode=='edit')
		$targetDir='ftp/sorties/'.$id_evt.'/'; // depuis la racine
	// création de sortie
	else
		$targetDir='ftp/user/'.intval($_SESSION['user']['id_user']).'/transit-nouvellesortie'; // depuis la racine

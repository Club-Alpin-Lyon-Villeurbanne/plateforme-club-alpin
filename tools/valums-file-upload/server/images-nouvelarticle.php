...
if($mode=='edit' && !$id_article) $errTab[]="ID sortie manquant";

if(!sizeof($errTab)){

	// creation des dossiers utiles pour l'user s'ils n'existnent pas
	$dir='../../../ftp/user/'.intval($_SESSION['user']['id_user']);
	if(!file_exists($dir)) mkdir($dir);
	$dir='../../../ftp/user/'.intval($_SESSION['user']['id_user']).'/transit-nouvelarticle';
	if(!file_exists($dir)) mkdir($dir);
	
	// modification de sortie
	if($mode=='edit')	$targetDir='ftp/articles/'.$id_article; // depuis la racine
	// création de sortie
	else				$targetDir='ftp/user/'.intval($_SESSION['user']['id_user']).'/transit-nouvelarticle'; // depuis la racine
	
	$targetDirRel='../../../'.$targetDir; // chemin relatif
	if(!file_exists($targetDirRel)) mkdir($targetDirRel);
	$targetDir.='/';
	$targetDirRel.='/';
...
if(!sizeof($errTab)){
	...
	if(!sizeof($errTab)){
		...
		if($size[0]/665 > $size[1]/365){		$W_max=0;	$H_max=365;		} // alors redimensionne en hauteur pour cropper les bords latéraux ensuite
		else{									$W_max=665;	$H_max=0;		} // sinon l'inverse : on crope les bords haut & bas
		// redimension
		...
		// plus large que haute, proportionnellement aux dimensions voulues ?
		if($size[0]/198 > $size[1]/138){		$W_max=0;	$H_max=138;		} // alors redimensionne en hauteur pour cropper les bords latéraux ensuite
		else{									$W_max=198;	$H_max=0;		} // sinon l'inverse : on crope les bords haut & bas
		// redimension
		$rep_Dst=$targetDirRel;

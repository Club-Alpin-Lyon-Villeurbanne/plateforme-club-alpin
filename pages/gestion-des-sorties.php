	...
	<div id="left1">
		<div class="main-type">
			...
			if(!allowed('evt_validate')){
				...
				// liste :
				if(!$notif_validerunesortie) echo '<p class="info">Aucune sortie n\'est en attente de publication pour l\'instant.</p>';
				else{
							...
							echo '<tr>'
									...
									.'<td>'
										...
										.'<div class="evt-tools">'
											...
											// apercu
											.'<a class="nice2" href="sortie/'.html_utf8($evt['code_evt']).'-'.intval($evt['id_evt']).'.html?forceshow=true" title="Ouvre une nouvelle fenêtre de votre navigateur pour jeter un oeil à la page avant publication" target="_blank">Aperçu</a> ';
					...
					for($i=0; $i<sizeof($evtStandby); $i++){
						$evt=$evtStandby[$i];
						if($i%2) $pair=true; else $pair=false;

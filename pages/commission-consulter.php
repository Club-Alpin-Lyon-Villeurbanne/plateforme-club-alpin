...
<div id="main" role="main" class="bigoo" style="">
	...
	<div id="left1">
		<div class="main-type">
			...
			if(!(admin() || allowed('comm_edit') || in_array('Resp. de commission, '.$code_commission, $_SESSION['user']['status']))) echo '<p class="erreur">Vous n\'avez pas les droits nécessaires pour afficher cette page</p>';
			else{
				...
				if(!$commissionTmp) echo '<p class="erreur"> ID invalide</p>';
					...
					while($row=$result->fetch_assoc()){
						...
						array_push ( $benvoles_emails , $row['email_user'] );
					}
					echo '</tbody></table>';

					echo '<h1>LISTE DES E-MAILS</h1>';
					echo '<textarea id="emailsaddresses" rows="10" cols="70">'.join (',', $benvoles_emails).'</textarea>';

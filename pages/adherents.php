...
<div id="main" role="main" style="width:100%">
	<div style="padding:20px 10px;">
		<?php
		if(!allowed('user_see_all')) echo '<p class="erreur">Vous n\'avez pas les droits nécessaires pour accéder à cette page</p>';
		else{
			?>
			...
			<p><strong>Voir les adhérents :</strong>
				<a href="adherents.html" <?php if($show=='valid') echo 'style="background:#d3d6ff"';?> class="boutonFancy"> compte activé / licence valide </a>&nbsp;
				<a href="adherents.html?show=valid-expired" <?php if($show=='valid-expired') echo 'style="background:#d3d6ff"';?> class="boutonFancy"> compte activé / licence expirée </a>&nbsp;
				<a href="adherents.html?show=notvalid" <?php if($show=='notvalid') echo 'style="background:#d3d6ff"';?> class="boutonFancy"> compte non activé / licence valide </a>&nbsp;
				<a href="adherents.html?show=expired" <?php if($show=='expired') echo 'style="background:#d3d6ff"';?> class="boutonFancy"> licence expirée </a>&nbsp;
				<a href="adherents.html?show=dels" <?php if($show=='dels') echo 'style="background:#d3d6ff"';?> class="boutonFancy"> désactivés manuellement </a>&nbsp;
				<a href="adherents.html?show=manual" <?php if($show=='manual') echo 'style="background:#d3d6ff"';?> class="boutonFancy"> créés manuellement </a>&nbsp;
				<a href="adherents.html?show=nomade" <?php if($show=='nomade') echo 'style="background:#d3d6ff"';?> class="boutonFancy"> nomades </a>&nbsp;
				<a href="adherents.html?show=all" <?php if($show=='all') echo 'style="background:#d3d6ff"';?> class="boutonFancy"> tous (+long) </a>
			</p>
...
//								if($elt['valid_user']){
									...
									if(allowed('user_giveright_1') || allowed('user_giveright_2') || allowed('user_givepresidence'))
										echo '<a href="includer.php?p=pages/adherents-droits.php&amp;id_user='.intval($elt['id_user']).'&amp;nom='.urlencode($elt['civ_user'].' '.$elt['firstname_user'].' '.$elt['lastname_user']).'" class="fancyframe" title="Voir / Attribuer des statuts à cet utilisateur"><img src="img/base/user_star.png" alt="RIGHTS" title=""></a> ';
...
//								}
							...
							.'<td>'.(allowed('user_read_private')?'<span style="display:none">'.$elt['birthday_user'].'</span>'.($elt['birthday_user']?intval($elt['birthday_user']).' ans':'...') : $img_lock).'</td>'

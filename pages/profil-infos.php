...
if(user()){
    ...
    <div class="main-type">
        ...
        <!-- Infos : statuts -->
        <?php if(sizeof($_SESSION['user']['status'])){ ?>
        ...
        <form id="user_update" class="contenutype2 loading" action="<?php echo $versCettePage;?>#user_update" method="post" enctype="multipart/form-data">
            ...
            <?php
            // TABLEAU
            if($_POST['operation'] == 'user_update' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
            if($_POST['operation'] == 'user_update' && !sizeof($errTab))	echo '<p class="info">Mise à jour effectuée à '.date("H:i:s", $p_time).'.</p>';
            ?>
            &nbsp;
            ...
            -->
            <h2><a href="/includer.php?p=includes/fiche-profil.php&id_user=<?php echo intval($_SESSION['user']['id_user']); ?>" class="fancyframe" title="Aperçu de votre fiche"><?php echo html_utf8($tmpUser['nickname_user']);?></a></h2>
            <?php
            ...
            <!--
            <div>
                <label for="gender_male"><input type="radio" name="civ_user" value="M." id="gender_m" <?php if($tmpUser['civ_user']=='M.') echo 'checked="checked"';?> />M.</label>
                <label for="gender_mm"><input type="radio" name="civ_user" value="Mme." id="gender_mm" <?php if($tmpUser['civ_user']=='Mme.') echo 'checked="checked"';?> />Mme.</label>
                <label for="gender_mlle"><input type="radio" name="civ_user" value="Mlle." id="gender_mlle" <?php if($tmpUser['civ_user']=='Mlle.') echo 'checked="checked"';?> />Mlle.</label>
                <label for="gender_unknown"><input type="radio" name="civ_user" value="..." id="gender_unknown" <?php if($tmpUser['civ_user']=='...') echo 'checked="checked"';?> />...</label>
            </div>
            -->
            <br style="clear:both" />
            <hr />
        ...
        <div style="float:left; width:90%; padding-right:5%">
                ...
                <?php
                    ...
                    if($tmpUser['alerte_renouveler_user'])	echo '<span class="alerte">';
            ...
            <b><?php
                ...
                echo "&nbsp;&nbsp;&nbsp;";
                echo html_utf8($tmpUser['pays_user']);
            ?></b>
            ...
            */ ?>
            ...
            <?php
                ...
                if ($ecriture) { display_niveaux($ecriture, 'ecriture'); }
                $lecture = get_niveaux($tmpUser['id_user'], false);
                if ($lecture) { display_niveaux($lecture, 'lecture', $ecriture); }
            ?>
            <hr />

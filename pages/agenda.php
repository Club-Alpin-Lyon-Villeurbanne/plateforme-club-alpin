	...
	<div id="left1">
		...
		<div style="padding:30px 20px 20px 20px">
			...
			<h1 class="agenda-h1">
				Agenda
				<?php
				if($current_commission)	echo ' : '.$comTab[$current_commission]['title_commission'];
				?>
			...
			<div class="faux-select-wrapper" style="float:left;">
				<div class="faux-select faux-select-wide">
					...
					echo '<a href="agenda.html?month='.intval($month).'&amp;year='.intval($year).'" title="" class="'.($current_commission?'':'up').'">Toutes les sorties</a> ';
					// choix de la commission
					ksort($comTab);
					foreach($comTab as $code=>$data){
						echo '<a href="agenda/'.html_utf8($code).'.html?month='.intval($month).'&amp;year='.intval($year).'" title="" class="'.($code==$current_commission?'up':'').'">'.html_utf8($data['title_commission']).'</a> ';
					}
					?>
				</div>
			</div>
			...
			<table id="agenda">
				...
				$weekTab=array('Dim', 'Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam');
				...
				for($i=1; $i<=$nDays; $i++){
					...
					echo '<tr class="'.(sizeof($agendaTab[$i]['debut'])?'up':'off').' '.$bgwe.'">' // ligne UP ou OFF si sortie démarre ou pas
							.'<td class="agenda-gauche '.$bgwe.'">'.$weekTab[$iWeek].' '.$i.' '.mois($month).'</td>'
							.'<td>'
							;

						// affichage des sorties commençantes
						for($j=0; $j<sizeof($agendaTab[$i]['debut']); $j++){
							if(!$first) echo '<hr />';
							$first=false;
						...
						for($j=0; $j<sizeof($agendaTab[$i]['courant']); $j++){
							if(!$first) echo '<hr />';
							$first=false;
							$evt=$agendaTab[$i]['courant'][$j];
							include INCLUDES.'agenda-evt-courant.php';
						}

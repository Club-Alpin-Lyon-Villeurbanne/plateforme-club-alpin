...
if(($currentPage['admin_page'] and !admin()) or ($currentPage['superadmin_page'] and !superadmin())){
	...
	function listPages($tab, $parent, $level){
		...
		for($i=0; $i<sizeof($tab); $i++){
			...
			if($page['parent_page']==$parent){
				...
				if($p_sublevels>$level+1) listPages($tab, $page['id_page'], $level+1);
			}
		}
	}
	?>
	...
	<form action="<?php echo $versCettePage;?>" method="post" class="loading">
		...
		<?php
		// TABLEAU
		if($_POST['operation'] == 'page_add' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>- '.implode('</li><li>- ', $errTab).'</li></ul></div>';
		if($_POST['operation'] == 'page_add' && !sizeof($errTab)){
			echo '<p class="info">Mise à jour effectuée à '.date("H:i:s", $p_time).'. Vous pouvez actualiser cette page</p>';
		...
		*/ 
		...
		<div class="buttonset">
			<input type="radio" id="menu_page_0" name="menu_page" value="0" <?php if($_POST['menu_page']=='0' or !$_POST) echo 'checked="checked"';?> /><label for="menu_page_0"> <img src="img/base/chart_organisation_delete.png" style="border-radius:3px; background:white; padding:3px; float:left; position:relative; bottom:3px; right:3px; " alt="" title="" /> Absent du menu principal</label>
			<input type="radio" id="menu_page_1" name="menu_page" value="1" <?php if($_POST['menu_page']=='1') echo 'checked="checked"';?> /><label for="menu_page_1"> <img src="img/base/chart_organisation_add.png" style="border-radius:3px; background:white; padding:3px; float:left; position:relative; bottom:3px; right:3px; " alt="" title="" /> Apparaît dans le menu principal</label>
		</div>
		<br />
		
		<div id="infomenu" style="display:<?php if($_POST['menu_page']==1) echo 'block'; else echo 'none';?>">
		...
		*/
		...
		<input type="text" name="priority_page" readonly="readonly" id="slideamount" value="<?php echo $_POST?intval($_POST['priority_page']):'5';?>" style="border:none; background:none; text-align:right; width:30px"></b><sub>/10</sub><br />
	...
	$().ready(function(){
		...
		$("#slider").slider({
			...
			min: 0,
			max: 10,
			value: <?php echo $_POST?intval($_POST['priority_page']):'5';?>,
			step: 1,

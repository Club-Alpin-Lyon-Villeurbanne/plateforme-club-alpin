...
if(!admin()){
	...
	$handleSql=$mysqli->query($req);
	while($handle=$handleSql->fetch_array(MYSQLI_ASSOC)){
		array_push($contGroupTab, $handle);
	}
	...
	<h2 style="padding-left:0px">
		Contenus 
		<?php if(sizeof($p_lang)>1) echo ' en langue principale ('.$p_langs[0].')'; ?>
	</h2>
	...
	<?php if(sizeof($p_langs)>1) echo "<p>Pour traduire les contenus dans d'autres langues, rendez-vous sur la page <b>traductions</b>.</p>"; ?>
	...
	<br />
	<!-- nouvel elt -->
	<form class="toggleForm add" action="<?php echo $versCettePage;?>" method="post" style="display:<?php if(($_POST['operation']=='addContentInline' && sizeof($errTab)) or $_POST['operation']=='forceAddContent') echo 'block';?>">
		...
		if($_POST['operation'] == 'addContentInline' && sizeof($errTab)) echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
		?>
		<h4>Ajouter un contenu</h4>
		<?php
		if(!sizeof($contGroupTab)) echo "Vous devez d'abord créer un groupe";
		else{
			?>
				...
				<tr>
					...
					<td>
						<select name="groupe_content_inline" style="min-width:150px;">
							...
							for($i=0; $i<sizeof($contGroupTab); $i++){
	...
	<!-- nouveau groupe -->
	<form class="toggleForm addgroup" action="<?php echo $versCettePage;?>" method="post" style="display:<?php if($_POST['operation']=='addContentGroup') echo 'block';?>">
		<input type="hidden" name="operation" value="addContentGroup" />
		<?php
		if($_POST['operation'] == 'addContentGroup' && !sizeof($errTab)) echo '<div class="info">Nouveau groupe créé, et disponible dans la liste.</div>';
		if($_POST['operation'] == 'addContentGroup' && sizeof($errTab)) echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
		?>
	...
	if(sizeof($contTab)){
		...
		<table class="cont-table">
			...
			for($i=0; $i<sizeof($contTab); $i++){
				...
				echo '<tr style="'.($dejaVus?'display:none':'').'" id="ligne-'.intval($contTab[$i]['id_content_inline']).'">';
				echo '<td class="cont-indice">'.$contTab[$i]['code_content_inline'].'&nbsp;</td>';
				echo '<td class="cont-edit">
						<input type="hidden" class="jId" value="'.intval($contTab[$i]['id_content_inline']).'" />
						<input type="text" style="display:none" class="jBase" id="base-'.intval($contTab[$i]['id_content_inline']).'" value="'.html_utf8(($contTab[$i]['contenu_content_inline'])).'" />
				...
				echo '<td class="cont-save"><a href="javascript:void(0)" title="Sauvegarder cette ligne" rel="'.intval($contTab[$i]['id_content_inline']).'"><img src="img/base/save.png" alt="Sauvegarder cette ligne" title="Sauvegarder cette ligne" class="upimage" style="height:20px; " /></a></td>';
	...
	function saveLine(id, anim){
		...
		$.ajax({
			...
			async: false,
			dataType: 'json',
			url: "index.php?ajx=contenus_save",
	...
	$().ready(function(){
		...
		<?php
		if($_POST['operation']=='forceAddContent'){
			?>

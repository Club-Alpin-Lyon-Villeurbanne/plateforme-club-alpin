...
<div id="main" role="main" style="width:100%">
	<div style="padding:20px 10px;">
		...
		echo '<p>';
		if(allowed('stats_commissions_read')) echo '<a href="/stats/commissions.html" '.($p2=='commissions'?'style="background:#d3d6ff"':'').' class="boutonFancy">Statistiques par sorties</a> ';
		if(allowed('stats_users_read')) echo '<a href="/stats/users.html" '.($p2=='users'?'style="background:#d3d6ff"':'').' class="boutonFancy">Statistiques par adhérents</a> ';
		if(allowed('article_create')) echo '<a href="/stats/nbvues.html" '.($p2=='nbvues'?'style="background:#d3d6ff"':'').' class="boutonFancy">Statistiques articles</a> ';
		echo '</p>';

		include SCRIPTS.'connect_mysqli.php';


		if((allowed('stats_commissions_read') || allowed('stats_users_read')) && ($p2 == 'commissions' || $p2 == 'users')) {
			?>
			...
			// par défaut :
			if(date('m') < 9) $dateMin = date('d/m/Y', mktime(0,0,0, 9, 1, date('Y')-1)); // départ au premier septebmre de l'année d'avant
			else $dateMin = date('d/m/Y', mktime(0,0,0, 9, 1, date('Y'))); // départ au premier septebmre cette année
			if(date('m') < 9) $dateMax = date('d/m/Y', mktime(0,0,0, 9, 1, date('Y'))); // fin au premier septebmre de Cette année
			else $dateMax = date('d/m/Y', mktime(0,0,0, 9, 1, date('Y')+1)); // fin au premier septebmre de l'année prochaine

			// recus :
			if(preg_match("#[0-9]{2}/[0-9]{2}/[0-9]{4}#", $_GET['dateMin']))	$dateMin = $_GET['dateMin'];
			if(preg_match("#[0-9]{2}/[0-9]{2}/[0-9]{4}#", $_GET['dateMax']))	$dateMax = $_GET['dateMax'];
			...
			if($p2 == 'users'){
				...
				foreach($comTab as $key => $comm){
					...
					AND cancelled_evt = 0
					AND commission_evt =".intval($comm['id_commission'])."
					...
					<tbody>
						...
						foreach($comTab as $key => $comm){
							...
							echo '<tr id="tr-'.$comm['id_commission'].'" class="'.($comm['vis_commission']?'vis-on':'vis-off').'">'
								...
								.'<td>'.intval($comm['stats']['join_total']).'</td>'
								.'<td>'.intval($comm['stats']['join_1']).'</td>'
								.'<td>'.intval($comm['stats']['join_2']).'</td>'
								.'<td>'.intval($comm['stats']['join_total_hommes']).'</td>'
								.'<td>'.intval($comm['stats']['join_total_femmes']).'</td>'
								.'<td>'.($comm['stats']['join_total']>0 ? intval($comm['stats']['join_total_hommes']*100 / $comm['stats']['join_1']) :'0').'</td>'

								.'<td>'.intval($comm['stats']['join_total_mineurs']).'</td>'
								.'<td>'.($comm['stats']['join_total']>0 ? intval($comm['stats']['join_total_mineurs']*100 / $comm['stats']['join_1']) :'0').'</td>'

								.'<td>'.intval($comm['stats']['join_3']).'</td>'
							. '</tr>'."\n";
						}
				...
				<table id="statsCommissions" class="datatables ">
					...
					<tbody>
						...
						foreach($comTab as $key => $comm){
							...
							echo '<tr id="tr-'.$comm['id_commission'].'" class="'.($comm['vis_commission']?'vis-on':'vis-off').'">'
								.'<td>'.html_utf8($comm['title_commission']).'</td>'
								.'<td>'.intval($comm['stats']['evt_total']).'</td>'
								.'<td>'.intval($comm['stats']['evt_1']).'</td>'
								.'<td>'.intval($comm['stats']['evt_2']).'</td>'
								.'<td>'.intval($comm['stats']['evt_legal_1']).'</td>'
								.'<td>'.intval($comm['stats']['evt_legal_0']).'</td>'
								.'<td>'.intval($comm['stats']['respcomm']).'</td>'
								.'<td>'.intval($comm['stats']['encadrants']).'</td>'
								.'<td>'.intval($comm['stats']['coencadrants']).'</td>'
							. '</tr>'."\n";
						}
		...
		} elseif (allowed('article_create') && $p2 == 'nbvues' ) {
?>
				<br />
	...
	<?php
			...
			<?php
					while ($article = $result->fetch_assoc()) {
						...
						. '<td><a href="article/'.html_utf8($article['code_article']).'-'.intval($article['id_article']).'.html" target="_blank">'.$article['titre_article'].'</a></td>'

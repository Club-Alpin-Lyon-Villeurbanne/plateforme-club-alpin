...
if(!admin()){
	...
	// langue en cours
	if(in_array($p2, $p_langs) && $p2!=$p_langs[0])
	...
	if(sizeof($contTab)){
		...
		<table class="cont-table">
			...
			for($i=0; $i<sizeof($contTab); $i++){
				...
				echo '<tr class="saveAble" style="'.($dejaVus?'display:none':'').'" id="ligne-'.intval($contTab[$i]['id_content_inline']).'">';
				...
				echo '<td class="cont-edit">
						...
						<input type="hidden" class="jGroupe" value="'.intval($contTab[$i]['groupe_content_inline']).'" />
	...
	function saveLine(ligne, anim){
		...
		$.ajax({
			...
			async: false,
			dataType: 'json',
			url: "index.php?ajx=traductions_save",
			data: datas,

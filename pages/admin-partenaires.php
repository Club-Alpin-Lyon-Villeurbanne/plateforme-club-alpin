...
if(!admin()){
	...
	<strong>Voir les partenaires :</strong>
		<a href="admin-partenaires.html?show=all" <?php if($show=='all') echo 'style="background:#d3d6ff"';?> class="boutonFancy">&nbsp;tous&nbsp;</a>
		<a href="admin-partenaires.html?show=enabled" <?php if($show=='enabled') echo 'style="background:#d3d6ff"';?> class="boutonFancy">&nbsp;activé&nbsp;</a>&nbsp;
		<a href="admin-partenaires.html?show=disabled" <?php if($show=='disabled') echo 'style="background:#d3d6ff"';?> class="boutonFancy">&nbsp;désactivé&nbsp;</a>&nbsp;
		<a href="admin-partenaires.html?show=private" <?php if($show=='private') echo 'style="background:#d3d6ff"';?> class="boutonFancy">&nbsp;privé&nbsp;</a>&nbsp;
		<a href="admin-partenaires.html?show=public" <?php if($show=='public') echo 'style="background:#d3d6ff"';?> class="boutonFancy">&nbsp;public&nbsp;</a>&nbsp;
	</p>
...
//			"aaSorting": [[ 2, "desc" ], [ 4, "asc" ]],
	...
	<table id="pagesLibres" class="datatables ">
		...
		<tbody>
			...
			for($i=0; $i<sizeof($partenairesTab); $i++){
				$elt=$partenairesTab[$i];

				echo '<tr id="tr-'.$elt['part_id'].'" class="'.($elt['part_enable']?'vis-on':'vis-off').'">'
					.'<td style="white-space:nowrap;">';
					// edit
					echo '<a href="/includer.php?p=pages/partenaire-modifier.php&amp;part_id='.intval($elt['part_id']).'" class="fancyframe" title="Modifier ce partenaire"><img src="img/base/application_form_edit.png" alt="MODIFIER" title=""></a> ';
					echo '&nbsp;&nbsp;&nbsp;<a href="/includer.php?p=pages/partenaire-supprimer.php&amp;part_id='.intval($elt['part_id']).'" class="fancyframe" title="Supprimer"><img src="img/base/delete.png" alt="SUPPRIMER" title="SUPPRIMER"  style="margin-bottom:-2px;"></a> ';

					if($elt['part_enable']==1){
						// desactiver
...
//						echo '<a href="/includer.php?p=pages/partenaire-enable.php&amp;part_id='.intval($elt['part_id']).'" class="fancyframe" title="Activer ce partenaire"><img src="img/base/add.png" alt="ACTIVER" title=""></a> ';
					...
					.'<td>'.($elt['part_enable']==1?'ACTIF':'INACTIF').'</td>'
					...
					echo '</a></td>'
					.'<td>'.($elt['part_type']==1?'PRIVÉ':'PUBLIC').'</td>'

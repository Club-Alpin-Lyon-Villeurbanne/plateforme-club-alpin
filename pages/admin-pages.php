...
if(($currentPage['admin_page'] and !admin()) or ($currentPage['superadmin_page'] and !superadmin())){
	...
	<div class="sortablepagelist niv0">
		...
		while($handle=$handleSql->fetch_array(MYSQLI_ASSOC)){
			...
			for($i=0; $i<sizeof($p_langs); $i++){	
				...
				while($handle2=$handleSql2->fetch_array(MYSQLI_ASSOC))	$tmp=$handle2['contenu_content_inline'];
	...
	$().ready(function(){
		...
		for(i=0; i<=<?php echo intval($p_sublevels); ?>; i++){
			$('.sortablepagelist.niv'+i).sortable({
				...
				stop:function(i){
					$.ajax({
						type: "GET",
						url: "index.php?ajx=pages_reorder",
						data: getDatas()
					})
				}
			});

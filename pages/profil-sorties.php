...
if(user()){
	...
	<div class="main-type">
		...
		// Rien ?
		if(!sizeof($evtList)) echo '<p class="info">Vous n\'avez pas encore de sortie à afficher ici.</p>';
		else echo '<p class="mini">'.$total.' sortie'.($total>1?'s':'').'</p>';
		...
		if(sizeof($evtList)) { ?>
			...
			<table id="agenda">
			<?php
				for($i=0; $i<sizeof($evtList); $i++) {
					$evt = $evtList[$i];

                    $empiete = empietement_sortie($_SESSION['user']['id_user'], $evt);

                    $status_evt=  ($evt['status_evt']==0 && $evt['user_evt']==$_SESSION['user']['id_user']?'<p class="alerte">Sortie en attente de publication</p>':'')
                        .($evt['status_evt']==1 && $evt['user_evt']==$_SESSION['user']['id_user']?'<p class="info">Sortie publiée sur le site</p>':'')
                        .($evt['status_evt']==2 && $evt['user_evt']==$_SESSION['user']['id_user']?'<p class="erreur">Sortie refusée et non publiée</p>':'');
					...
					echo '<tr>'
							.'<td class="agenda-gauche">'
                                ...
                                ($evt['destination']?
                                    ($evt['destination']['publie'] == 0 ? '<p class="alerte">Destination en attente de publication</p>' : $status_evt)
                                    :$status_evt)
							.'</td>'
							.'<td>';
                        include INCLUDES.'evt-tools.php';

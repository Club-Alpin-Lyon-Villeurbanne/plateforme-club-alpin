	...
	<div id="left1">
		<div class="main-type">
			...
			if(user()){
                ...
                if ($evt) $joins = $evt['joins'];
                if ($destination) $joins = $destination['joins'];

                if ($p2 == 'destination') {

                    // destination non trouvée, pas de message d'erreur, équivalent à un 404
                    if(!$destination && !$errPage) echo '<br /><br /><br /><p class="erreur">Hmmm... C\'est ennuyeux : nous n\'arrivons pas à trouver la destination correspondant à cette URL.</p>';
                    // destination non trouvée, avec message d'erreur, tentative d'accès mesquine ou sortié dévalidée
                    if(!$destination && $errPage) echo '<div class="erreur">'.$errPage.'</div>';
                    ...
                    if($destination && !$errPage){
                        ...
                        inclure($p1, 'vide');
                        inclure($p1.'-'.$p2, 'vide');
                        if($_POST['operation'] == 'dest_cancel' && sizeof($errTab))	{
                            ...
                            if($destination['annule']!='1'){
                                ...
                                <form action="<?php echo $versCettePage;?>" method="post" class="loading">
                                    ...
                                    <?php
                                    // TABLEAU
                                    if($_POST['operation'] == 'dest_cancel' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
                                    if($_POST['operation'] == 'dest_cancel' && !sizeof($errTab))	echo '<p class="info">Cette destination a été annulée.</p>';
                                    ?>
                                    <br />
                                    ...
                                    if($destination['publie']==1) {
                                        ...
                                        <a href="javascript:void(0)" title="Enregistrer" class="nice2 red" onclick="$(this).parents('form').submit()">
                                            Annuler définitivement la destination ci-dessous et avertir <?php echo sizeof($joins); ?> participant(s) inscrit(s).
                                        </a>
                                    <?php
                                    }
                    ...
                    if(!$evt && !$errPage) echo '<br /><br /><br /><p class="erreur">Hmmm... C\'est ennuyeux : nous n\'arrivons pas à trouver la sortie correspondant à cette URL.</p>';
                    // sortie non trouvée, avec message d'erreur, tentative d'accès mesquine ou sortié dévalidée
                    if(!$evt && $errPage) echo '<div class="erreur">'.$errPage.'</div>';
                    ...
                    if($evt && !$errPage){
                        ...
                        <?php
                        inclure($p1, 'vide');
                        if($_POST['operation'] == 'evt_cancel' && sizeof($errTab))	{
                            echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div><br /><br />';
                            ...
                            if($evt['cancelled_evt']!='1'){
                                ...
                                <form action="<?php echo $versCettePage;?>" method="post" class="loading">
                                    ...
                                    <?php
                                    // TABLEAU
                                    if($_POST['operation'] == 'evt_cancel' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
                                    if($_POST['operation'] == 'evt_cancel' && !sizeof($errTab))	echo '<p class="info">Cette sortie a été annulée.</p>';
                                    ?>
                                    <br />
                                    ...
                                    if($evt['status_evt']==1) {
                                        ...
                                        <a href="javascript:void(0)" title="Enregistrer" class="nice2 red" onclick="$(this).parents('form').submit()">
                                            Annuler définitivement la sortie ci-dessous et avertir <?php echo sizeof($joins); ?> participant(s) inscrit(s).
                                        </a>
                                        <?php
                                    }
                                    // sinon le message n'est pas necessaire
            ...
            <?php
                ...
                <?php
                // RESUME DE LA SORTIE
                if ($evt) include INCLUDES.'evt-resume.php';
                if ($destination) include INCLUDES.'dest'.DS.'display.php';
                ?>

		...
		<table width='100%'>
			...
			<?php
				...
				if(admin() && !is_array($userTab['sorties']) && !is_array($userTab['articles'] )) {
					$rowValue .= '&nbsp;&nbsp;&nbsp;<a href="includer.php?p=pages/adherents-supprimer.php&amp;id_user='.intval($userTab['id_user']).'&amp;nom='.urlencode($userTab['civ_user'].' '.$elt['firstname_user'].' '.$userTab['lastname_user']).'" title="Supprimer le compte de cet utilisateur"><img src="img/base/user_delete.png" alt="SUPPRIMER" title=""></a> ';
				...
				if($userTab['valid_user'] == 0 && strlen($userTab['cookietoken_user'])>0){
					$rowValue .= '<br />URL d\'activation du compte : ';
					if(admin()) $rowValue .= '<a href="'.$p_racine.'user-confirm/'.$userTab['cookietoken_user'].'-'.$userTab['id_user'].'.html">';
				...
				if($userTab['cafnum_parent_user']) {
					$rowValue = '<a href="includer.php?p=pages/adherents-consulter.php&amp;id_user='.intval($userTab['cafnum_parent_user']['id_user']).'">'.$userTab['cafnum_parent_user']['firstname_user'].' '.$userTab['cafnum_parent_user']['lastname_user'].'</a>';
				...
				if(is_array($userTab['enfants'])){
					foreach ($userTab['enfants'] as $enfant){
						$rowValue[] = '<a href="includer.php?p=pages/adherents-consulter.php&amp;id_user='.intval($enfant['id_user']).'">'.$enfant['firstname_user'].' '.$enfant['lastname_user'].'</a>';
					}

					printTableRow('Adhérents affiliés : ', join ('<br />', $rowValue));
				}

				$rowValue ='NC';

						if ($userTab['date_adhesion_user'] > 0) $rowValue = date("d/m/Y", $userTab['date_adhesion_user']);
				...
				if ($userTab['birthday_user']) printTableRow('Date de naissance :', date("d/m/Y",$userTab['birthday_user']) .'&nbsp;&nbsp;&nbsp;('. getYearsSinceDate($userTab['birthday_user']) .' ans)');
				if ($userTab['email_user']) printTableRow('E-mail :', '<a href="mailto:'. $userTab['email_user'].'">'.$userTab['email_user'].'</a>');
				if ($userTab['tel_user']) printTableRow('Numéro de téléphone personnel :', $userTab['tel_user']);
				if ($userTab['tel2_user']) printTableRow('Numéro de téléphone de sécurité :', $userTab['tel2_user']);
				...
				printTableRow('Statut compte internet :', (($userTab['valid_user']==1)?'ACTIF':(($userTab['valid_user']==2)?'DESACTIVE':'NON ACTIF')));
				if ($userTab['created_user']) printTableRow('Création du compte :', date("d/m/Y",$userTab['created_user']));
				if ($userTab['ts_insert_user']) printTableRow('Insertion en base :', date("d/m/Y",$userTab['ts_insert_user']));
				if ($userTab['ts_update_user']) printTableRow('Mise à jour en base :', date("d/m/Y",$userTab['ts_update_user']));
				if (is_array($userTab['articles'])){
					$rowValue = array();
					foreach ($userTab['articles'] as $article){
						$rowValue[] = '<a href="article/'.html_utf8($article['code_article']).'-'.intval($article['id_article']).'.html?forceshow=true" target="_blank">'.date('d.m.Y', $article['tsp_validate_article']).' - '.$article['titre_article'].'</a>';
					}
					printTableRow('Articles :', '<font size="-1" >'.join ('<br />', $rowValue).'</font>');
				}
				...
				if (is_array($userTab['sorties'])) {
					...
					foreach ($userTab['sorties'] as $evt){
						...
						$row = '<a target="_blank" href="sortie/'.html_utf8($evt['code_evt']).'-'.intval($evt['id_evt']).'.html?commission='.$evt['code_commission'];
					...
					printTableRow('Sorties :<br />'.'<font size="-1" >'.$rowRoleEvtValue.'</font>', '<font size="-1" >'.join ('<br />', $rowValue).'</font>');
				}

		...
		<div class="main-type">
			...
			if(!strlen($_GET['str'])){
				...
				if(sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
					...
					// compte
					if(!sizeof($articlesTab)) echo '<p class="alerte">Aucun article trouvé</p>';
					else echo '<p class="mini">'.$totalArticles.' article'.($totalArticles>1?'s':'').' trouvé'.($totalArticles>1?'s':'').'. '.($totalArticles != sizeof($articlesTab) ? 'Voici les '.sizeof($articlesTab).' plus récents :':'').'</p>';
					...
					// compte
					if(!sizeof($evtTab)) echo '<p class="alerte">Aucune sortie trouvée</p>';
					else echo '<p class="mini">'.$totalEvt.' sortie'.($totalEvt>1?'s':'').' trouvée'.($totalEvt>1?'s':'').'. '.($totalEvt != sizeof($evtTab) ? 'Voici les '.sizeof($evtTab).' plus récentes :':'').'</p>';

...
<div id="main" role="main" class="bigoo" style="">
	...
	<div id="left1">
		<div class="main-type">
			...
			<?php
			if(!allowed('article_validate') and !allowed('article_validate_all')){
				...
				if(!$notif_validerunarticle) echo '<p class="info">Aucun article n\'est en attente de publication pour l\'instant.</p>';
				else{

					// ************
					// ** AFFICHAGE, on recupere le design de l'agenda
					for($i=0; $i<sizeof($articleStandby); $i++){
						...
						if($article['commission_article'] == 0) $type="Actualité du club (toutes les commissions)";
						elseif($article['commission_article'] == -1) $type="Compte rendu de sortie";
						else $type="Actualité « ".$article['title_commission']." »";
						...
						.'<div class="article-tools-valid">'
							...
							// apercu
							.'<a class="nice2" href="article/'.html_utf8($article['code_article']).'-'.intval($article['id_article']).'.html?forceshow=true" title="Ouvre une nouvelle fenêtre de votre navigateur pour jeter un oeil à la page avant publication" target="_blank">Aperçu</a> ';
							...
							<div style="display:none" id="refuser-'.intval($article['id_article']).'">
							...
							.'<div style="width:100px; float:left; padding:6px 10px 0 0;"><a href="article/'.html_utf8($article['code_article']).'-'.intval($article['id_article']).'.html?forceshow=true" target="_blank">'
								// image liee
							...
							.'<p style="padding:5px 5px; line-height:18px;">'
								.'<b><a href="article/'.html_utf8($article['code_article']).'-'.intval($article['id_article']).'.html?forceshow=true" target="_blank">'.html_utf8($article['titre_article']).'</a></b><br />'
				...
				$notif_validerunarticle = sizeof($articleStandbyRedac);
				if($notif_validerunarticle == 0) echo '<p class="info">Aucun article n\'est en cours de rédaction pour l\'instant.</p>';
				else{
					...
					// ************
					// ** AFFICHAGE, on recupere le design de l'agenda
					for($i=0; $i<sizeof($articleStandbyRedac); $i++){
						...
						.'<div class="article-tools-valid">'
							...
							// edition
							if (allowed('article_edit_notmine') or allowed('article_edit', 'commission:'.$article['commission_article'] )) {

								echo '<a href="article-edit/'.intval($article['id_article']).'.html" title="" class="nice2 orange">
									Modifier
								</a>';
							}

							// Suppression
							if (allowed('article_delete_notmine') or allowed('article_delete', 'commission:'.$article['commission_article'] )) {
								echo '<a href="javascript:$.fancybox($(\'#supprimer-form-'.$article['id_article'].'\').html());" title="" class="nice2 red">
										Supprimer
									</a>';
								echo '<div id="supprimer-form-'.intval($article['id_article']).'" style="display:none">
							...
							echo '<div style="width:100px; float:left; padding:6px 10px 0 0;"><a href="article/'.html_utf8($article['code_article']).'-'.intval($article['id_article']).'.html?forceshow=true" target="_blank">'
								// image liee
								.'<img src="'.$img.'" alt="" title="" style="width:100%; " />'
							.'</a></div>'
							.'<div style="float:right; width:510px">'

...
if(!admin()){
	...
	<div style="text-align:left;">
		...
		<form action="<?php echo $versCettePage;?>" method="post" onSubmit="javascript:top.$.fancybox.close()" >
			...
			<?php
			// TABLEAU
			if($_POST['operation'] == 'partenaire_delete' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
			if($_POST['operation'] == 'partenaire_delete' && !sizeof($errTab)) echo '<p class="info">Partenaire supprimé ! (Vous devrez <a href="javascript:top.$.fancybox.close();top.parent.location.reload(false);">Recharger la page</a> pour voir le changement)</p>';
			else{
				?>

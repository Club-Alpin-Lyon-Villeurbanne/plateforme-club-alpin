...
?>
	...
	<form action="<?php echo $versCettePage;?>" method="post">
		...
		<?php
		// TABLEAU
		if($_POST['operation'] == 'pagelibre_add' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>- '.implode('</li><li>- ', $errTab).'</li></ul></div>';
		if($_POST['operation'] == 'pagelibre_add' && !sizeof($errTab)){
		...
		<input type="text" name="priority_page" readonly="readonly" id="slideamount" value="<?php echo $_POST?intval($_POST['priority_page']):'50';?>" style="border:none; background:none; text-align:right; width:30px"></b><sub>/100 %</sub><br />
	...
	$().ready(function(){
		...
		$("#slider").slider({
			...
			min: 0,
			max: 100,
			value: <?php echo $_POST?intval($_POST['priority_page']):'50';?>,
			step: 10,

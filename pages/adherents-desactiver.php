...
if(!allowed('user_desactivate_any')){
	...
	<div style="text-align:left;">
		...
		<form action="<?php echo $versCettePage;?>" method="post">
			...
			<?php
			// TABLEAU
			if($_POST['operation'] == 'user_desactiver' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
			if($_POST['operation'] == 'user_desactiver' && !sizeof($errTab))	echo '<p class="info">Utilisateur désactivé (Vous devrez <a href="adherents.html" title="" target="_top">actualiser la page</a> pour voir le changement)</p>';
			else{
				?>

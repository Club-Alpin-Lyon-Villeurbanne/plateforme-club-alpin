...
<div id="main" role="main">
	<div style="padding:20px 10px;">
		<?php
		if(!allowed('user_updatefiles')) echo '<p class="erreur">Vous n\'avez pas les droits nécessaires pour accéder à cette page</p>';
		else{
			?>
			...
			<form action="<?php echo $versCettePage;?>" method="post" enctype="multipart/form-data" class="loading">
				...
				if($_POST['operation'] == 'fichier_adherents_maj' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
				if($_POST['operation'] == 'fichier_adherents_maj' && !sizeof($errTab))	echo '<p class="info">Mise à jour effectuée à '.date("H:i:s", $p_time).'.</p>';
				?>
			...
			$fileTab=array('ftp/fichiers-proteges/74800.txt');
			foreach($fileTab as $file){
				?>
				<h2>Fichier <?php echo strtolower(substr(strrchr($file, '/'), 1)); ?></h2>
				<?php
				if(!file_exists($file) || !is_file($file)) echo '<p class="erreur">Fichier introuvable !</p>';
				else{

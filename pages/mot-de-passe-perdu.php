	...
	if(!$p2){
		...
		<form action="<?php echo $versCettePage;?>" method="post" style="text-align:left;">
			...
			// TABLEAU
			if($_POST['operation'] == 'user_mdp_reinit' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
			if($_POST['operation'] == 'user_mdp_reinit' && !sizeof($errTab)) echo '<p class="info">OK, votre e-mail vient d\'être envoyé à '.date("H:i:s", $p_time).', sur '.html_utf8($email_user).'. <br />Courez vérifier votre boite e-mail.</p>';
			
			// en cas de succès, pas de suite au form
			if($_POST['operation'] != 'user_mdp_reinit' || sizeof($errTab)){
				?>

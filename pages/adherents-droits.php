...
if(!allowed('user_giveright_1') && !allowed('user_giveright_2') && !allowed('user_givepresidence')){
	...
	<div style="text-align:left;">
		...
		while($row = $result->fetch_assoc()) $statsTab[]=$row;
		...
		if(sizeof($statsTab)){
			...
			foreach($statsTab as $row){
				echo '<li>
						...
						if($row['code_usertype']=='benevole' && allowed('user_giveright_1', $row['params_user_attr'])) $deleteRight=true;
						if($row['code_usertype']=='encadrant' && allowed('user_giveright_1', $row['params_user_attr'])) $deleteRight=true;
						if($row['code_usertype']=='coencadrant' && allowed('user_giveright_1', $row['params_user_attr'])) $deleteRight=true;
						if($row['code_usertype']=='redacteur' && allowed('user_giveright_1', $row['params_user_attr'])) $deleteRight=true;
						
						// user_giveright_2
						if($row['code_usertype']=='salarie' && allowed('user_giveright_2')) $deleteRight=true;
						
						// user_givepresidence
						if($row['code_usertype']=='president' && allowed('user_givepresidence')) $deleteRight=true;
						if($row['code_usertype']=='vice-president' && allowed('user_givepresidence')) $deleteRight=true;
		...
		<form action="<?php echo $versCettePage;?>" method="post">
			...
			<?php
			// message
			if($_POST['operation'] == 'user_attr_add' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
			if($_POST['operation'] == 'user_attr_add' && !sizeof($errTab))	echo '<div class="info">Mise à jour effectuée à '.date("H:i:s", $p_time).'.</div>';
			...
			while($row = $result->fetch_assoc()){
				...
				// user_giveright_1 
				if($row['code_usertype']=='benevole' && allowed('user_giveright_1')) $afficher=true;
				if($row['code_usertype']=='coencadrant' && allowed('user_giveright_1')) $afficher=true;
				if($row['code_usertype']=='encadrant' && allowed('user_giveright_1')) $afficher=true;
				if($row['code_usertype']=='redacteur' && allowed('user_giveright_1')) $afficher=true;
				
				// user_giveright_2
				if($row['code_usertype']=='salarie' && allowed('user_giveright_2')) $afficher=true;
				
				// user_giveright_3
				if($row['code_usertype']=='responsable-commission' && allowed('user_giveright_3')) $afficher=true;
				
				// user_givepresidence
				if($row['code_usertype']=='president' && allowed('user_givepresidence')) $afficher=true;
				if($row['code_usertype']=='vice-president' && allowed('user_givepresidence')) $afficher=true;
				
				if($afficher)
					echo'<option value="'.intval($row['id_usertype']).'" class="precise-comm-'.intval($row['limited_to_comm_usertype']).'">'.html_utf8($row['title_usertype']).'</option>';

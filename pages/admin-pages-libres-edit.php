...
if(!admin()){
	...
	if(!$page) { echo '<div class="erreur">ID invalide</div>'; exit(); }
	?>
	...
	<form action="<?php echo $versCettePage;?>" method="post">
		...
		<?php
		// TABLEAU
		if($_POST['operation'] == 'pagelibre_edit' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>- '.implode('</li><li>- ', $errTab).'</li></ul></div>';
		if($_POST['operation'] == 'pagelibre_edit' && !sizeof($errTab)){
		...
		<input type="text" name="priority_page" readonly="readonly" id="slideamount" value="<?php echo intval($page['priority_page'])*100;?>" style="border:none; background:none; text-align:right; width:30px"></b><sub>/100 %</sub><br />
	...
	$().ready(function(){
		...
		$("#slider").slider({
			...
			min: 0,
			max: 100,
			value: <?php echo intval($page['priority_page'])*100;?>,
			step: 10,

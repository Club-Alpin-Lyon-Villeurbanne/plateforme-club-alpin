...
if(!admin()){
	...
	<form action="<?php echo $versCettePage;?>" method="post">
		...
		<?php
		// message
		if($_POST['operation'] == 'user_attr_add_admin' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
		if($_POST['operation'] == 'user_attr_add_admin' && !sizeof($errTab))	echo '<div class="info">Mise à jour effectuée à '.date("H:i:s", $p_time).'.</div>';

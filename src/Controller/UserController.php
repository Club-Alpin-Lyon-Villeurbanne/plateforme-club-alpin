...
class UserController extends AbstractController
{
    ...
    public function full(
        ...
        Mailer $mailer,
        FormationNiveauValidationRepository $formationNiveauRepository,
        FormationCompetenceValidationRepository $formationCompetenceValidationRepository,
    ): array {
        ...
        /** @var FormationNiveauValidation $niveau */
        foreach ($niveaux as $niveau) {
            $nivRefs[$niveau->getNiveauReferentiel()->getId()] = $niveau->getNiveauReferentiel();
            $code = $niveau->getNiveauReferentiel()->getCodeActivite();

            $commissionsNiveau = $manager->getRepository(Commission::class)->findBy(['codeFfcamNiveau' => $code]);
        ...
        /** @var FormationCompetenceValidation $groupesComp */
        foreach ($groupesComps as $groupesComp) {
            $groupesCompRefs[$groupesComp->getCompetence()->getId()] = $groupesComp->getCompetence();
            $code = $groupesComp->getCompetence()->getCodeActivite();

            $commissionsNiveau = $manager->getRepository(Commission::class)->findBy(['codeFfcamGroupeCompetence' => $code]);
            foreach ($commissionsNiveau as $commission) {
                $commissions[$commission->getId()] = $commission;
            }
        }

...
class ProfilController extends AbstractController
{
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($mediaUploadId) {
                ...
                if ($mediaUpload && $mediaUpload->getUploadedBy() === $this->getUser()) {
                    $user->setMediaUpload($mediaUpload);
    ...
    public function removeProfilPicture(Filesystem $filesystem, EntityManagerInterface $manager): RedirectResponse
    {
        /** @var User $user */
        $user = $this->getUser();
        $filesystem->remove($this->getParameter('kernel.project_dir') . '/public/ftp/uploads/files/' . $user->getMediaUpload()->getFilename());
        $manager->remove($user->getMediaUpload());
        $user->setMediaUpload(null);
        $manager->persist($user);
        $manager->flush();

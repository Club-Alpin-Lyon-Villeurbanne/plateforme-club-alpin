...
class HelloAssoWebhookController extends AbstractController
{
    ...
    public function notification(Request $request): Response
    {
        ...
        if ($this->helloAssoServerIp !== $request->getClientIp()) {
            ...
            return new Response('Invalid IP', Response::HTTP_OK);
        }
        ...
        if (!\in_array('x-ha-signature', array_keys($requestHeader), true)) {
            ...
            return new Response('Missing signature', Response::HTTP_OK);
        }
        ...
        // on retourne une 200 même en cas d'erreur pour ne pas que HelloAsso renvoie plusieurs fois la notification (inutile si l'erreur est de notre côté)

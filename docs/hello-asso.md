...
### Limitations connues
1. Bien qu'on puisse créer une campagne via API, elle reste à l'état de brouillon et il faut la publier manuellement depuis le dashboard @todo voir si c'est possible de la "publier" via API
2. Il ne semble pas possible de supprimer une campagne ou de la modifier via API, que faire en cas de modification de sortie si on souhaite enlever la billetterie ou modifier le montant ?

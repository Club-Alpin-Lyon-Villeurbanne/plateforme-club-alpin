...
if ($evt) {
    echo '<div class="evt-tools">'
        ...
        .($evt['user_evt'] == (string) getUser()->getIdUser() && 1 != $evt['status_evt'] ? '<a class="nice2" href="sortie/'.html_utf8($evt['code_evt']).'-'.(int) ($evt['id_evt']).'.html?forceshow=true'.($current_commission ? '&amp;commission='.$current_commission : '').'" title="Aperçu de la page dédiée à la sortie ci-dessous">Aperçu</a>' : '')

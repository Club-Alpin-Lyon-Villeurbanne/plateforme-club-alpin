...
if (!isset($errTab) || 0 === count($errTab)) {
    ...
    // sécurisation BDD
    $titre_evt = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($titre_evt);
    $tarif_evt = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($tarif_evt);
    $cb_evt = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($cb_evt);
    $tarif_detail = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($tarif_detail);
    $repas_restaurant = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($repas_restaurant);
    $tarif_restaurant = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($tarif_restaurant);
    $massif_evt = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($massif_evt);
    $rdv_evt = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($rdv_evt);
    $tsp_evt = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($tsp_evt);
    $tsp_end_evt = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($tsp_end_evt);
    $tsp_evt_day = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($tsp_evt_day);
    $tsp_evt_hour = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($tsp_evt_hour);
    $tsp_end_evt_day = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($tsp_end_evt_day);
    $tsp_end_evt_hour = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($tsp_end_evt_hour);
    $matos_evt = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($matos_evt);
    $itineraire = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($itineraire);
    $difficulte_evt = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($difficulte_evt);
    $description_evt = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($description_evt);

    if (0 == $id_groupe) {
        $id_groupe = 'NULL';
    } else {
        $id_groupe = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($id_groupe);
    }
    ...
    if ('evt_create' == $_POST['operation']) {
        ...
        if (0 == $cycle_parent_evt) {
            ...
            $kernel->getContainer()->get('legacy_mysqli_handler')->query($req2);
        }
    ...
    if (!$kernel->getContainer()->get('legacy_mysqli_handler')->query($req)) {
        $errTab[] = 'Erreur SQL creation/update : '.$kernel->getContainer()->get('legacy_mysqli_handler')->lastError();
    } else {
        // jointures de l'ev avec les users spécifiés (encadrant, coenc' benev')

        if ('evt_create' == $_POST['operation']) {
            $id_evt = $kernel->getContainer()->get('legacy_mysqli_handler')->insertId();
        }
        ...
        if (!isset($errTab) || 0 === count($errTab)) {
            ...
            if ($_POST['id_destination']) {
                ...
                if (!$depose) {
                    ...
                    $lieu_nom = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($depose['nom']);
                    $lieu_description = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($depose['description']);
                    $lieu_ign = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($depose['ign']);
                    $lieu_lat = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($depose['lat']);
                    $lieu_lng = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($depose['lng']);
                    ...
                    if (!$kernel->getContainer()->get('legacy_mysqli_handler')->query($sql)) {
                        ...
                        $id_lieu_depose = $_POST['lieu']['depose']['id'] = $kernel->getContainer()->get('legacy_mysqli_handler')->insertId();
                    }
                }
                ...
                if ($copy_depose_to_reprise) {
                    ...
                    if (!$reprise) {
                        ...
                        $lieu_nom = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($reprise['nom']);
                        $lieu_description = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($reprise['description']);
                        $lieu_ign = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($reprise['ign']);
                        $lieu_lat = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($reprise['lat']);
                        $lieu_lng = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($reprise['lng']);
                        ...
                        if (!$kernel->getContainer()->get('legacy_mysqli_handler')->query($sql)) {
                            ...
                            $id_lieu_reprise = $_POST['lieu']['reprise']['id'] = $kernel->getContainer()->get('legacy_mysqli_handler')->insertId();
                        }
                    }
                }

                if (!isset($errTab) || 0 === count($errTab)) {
                    $date_depose = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($_POST['lieu']['depose']['date_depose']);
                    $date_reprise = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString($_POST['lieu']['reprise']['date_reprise']);

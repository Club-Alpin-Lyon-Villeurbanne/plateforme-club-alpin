...
if (!isset($errTab) || 0 === count($errTab)) {
    $stmt = LegacyContainer::get('legacy_mysqli_handler')->prepare('UPDATE `caf_user` SET email_user=? WHERE id_user=?');
    $stmt->bind_param('si', $userTab['email_user'], $userTab['id_user']);
    $stmt->execute();
    $stmt->close();

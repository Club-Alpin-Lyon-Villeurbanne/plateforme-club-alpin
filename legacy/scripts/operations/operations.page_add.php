...
// $default_name_page=trim($kernel->getContainer()->get('legacy_mysqli_handler')->escapeString(stripslashes($_POST['default_name_page'])));
$meta_title_page = (int) ($_POST['meta_title_page']);
$priority_page = (int) ($_POST['priority_page']) / 100;
// $add_css_page=trim($kernel->getContainer()->get('legacy_mysqli_handler')->escapeString(stripslashes($_POST['add_css_page'])));
// $add_js_page=trim($kernel->getContainer()->get('legacy_mysqli_handler')->escapeString(stripslashes($_POST['add_js_page'])));
...
if (!isset($errTab) || 0 === count($errTab)) {
    for ($i = 0; $i < count($p_langs); ++$i) {
        ...
        $contenu_content_inline = $kernel->getContainer()->get('legacy_mysqli_handler')->escapeString(stripslashes($titreTab[$i]));

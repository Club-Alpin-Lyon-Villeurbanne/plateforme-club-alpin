...
if (!isset($errTab) || 0 === count($errTab)) {
    ...
    if ($row[0]) {
        ...
        $current_time = time();
        $stmt->bind_param('ssssssisssssss', $email_user, $mdp_user, $cafnum_user, $firstname_user, $lastname_user, $nickname_user, $current_time, $birthday_user, $tel_user, $tel2_user, $adresse_user, $cp_user, $ville_user, $pays_user);
        if (!$stmt->execute()) {
            $errTab[] = 'Erreur SQL';
        } else {

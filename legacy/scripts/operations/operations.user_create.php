...
if (!isset($errTab) || 0 === count($errTab)) {
    ...
    if ($row[0]) {
        ...
        $stmt->bind_param('ssssssssssssss', $email_user, $mdp_user, $cafnum_user, $firstname_user, $lastname_user, $nickname_user, $birthDate?->format('Y-m-d'), $tel_user, $tel2_user, $adresse_user, $cp_user, $ville_user, $pays_user, $auth_contact_user);
        if (!$stmt->execute()) {
            $errTab[] = 'Erreur SQL';
        } else {

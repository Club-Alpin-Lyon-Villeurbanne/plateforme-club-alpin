...
$p_racine='http://'.$_SERVER['HTTP_HOST'].substr($_SERVER['PHP_SELF'], 0, strrpos($_SERVER['PHP_SELF'], '/')+1);
...
// Transporteur :
$p_transporteurs = array(
    'hwc' => array(
        'email'=>'e.henke@herewecom.fr',
        'nom'=>'HWC WebTransfers'
    ),
    /* 'mac' => array(
        'email'=>'dev2@herewecom.fr',
        'nom'=>'MAC @ HWC'
    ) */
);

// ffcam
$p_ffcam = array('6900', '6976'); 
...
$p_modules=array();
// langues
$p_langs=array('fr'); // la langue par défaut en premier
...
$p_ftpallowed=array('gpx', 'kml', 'kmz', 'jpg', 'gif', 'jpeg', 'png', 'doc', 'docx', 'odt', 'pdf', 'avi', 'mov', 'mp3', 'rar', 'zip', 'txt', 'xls', 'csv', 'ppt', 'pptx', 'ai', 'psd', 'fla', 'swf', 'eps');
// tinymce vars
...
$p_ftp_masquer=array('index.php','.','..','.htaccess','Thumbs.db','transit','article','articles','commission','user','sorties','galeries','partenaires');
// dossiers et fichiers a ne pas supprimer (racine: ftp/)
$p_ftp_proteges=array('images','telechargements', 'transit', 'fichiers-proteges');

// couples question/reponses antispam
$p_as1=array("Combien font quatre plus cinq ?","Combien font quatre fois deux ?","Combien font trois plus deux ?","Combien font deux plus cinq ?","Combien font cinq moins un ?","Combien font six moins trois ?","Combien font deux plus quatre ?","Combien font zéro plus deux ?","Combien font deux moins un ?");
$p_as2=array(9,8,5,7,4,3,6,2,1);
...
$p_multilangue=sizeof($p_langs)<2?false:true;

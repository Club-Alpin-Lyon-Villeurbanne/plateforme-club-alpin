			...
			$.ajax({
				...
				success: function(responseJSON){
					if(responseJSON.success){
						...
						// reload page FTP avec le bon dossier
						var tab=destImg.split('/');
						tab[tab.length-1]='';
						var targetDir=tab.join('/');
						parent.document.getElementById('frameFtp').contentDocument.location.href='ftp.php?dossier='+targetDir;
					}
					else{
						alert('Erreur dans la réponse du serveur');
					}

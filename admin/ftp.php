...
if(!admin()){
	...
	if($_GET['mode']=='relatif') $mode='relatif';
	if($_GET['mode']=='absolu') $mode='absolu';
	if(!$mode) $mode='relatif';
	
	// recuperation du dossier
	$racine='../ftp/';
	if(!isset($_GET['dossier'])){	$dossier=$racine;		}
	else{							$dossier=$_GET['dossier'];	}
	...
	<body style="background:#DEDEDE; border:none; font-size:12px;">
		<div id="ftp-2">
			...
			<div id="ftp-2-arbo">
				...
				<div class="level top">
					<a href="../ftp/" title="Racine du dossier" class="dirlink <?php if($dossier=='../ftp/') echo 'selected';?>">Dossier FTP</a>
					<?php
					function arbo_read($dir, $level){
						...
						//while($file=readdir($opendir)){
							...
							if(is_dir($dir.$file) && !in_array($file, $p_ftp_masquer)){
								...
								if(!arbo_read($dir.$file.'/', $level+1)) echo '<span class="removetrigger"></span>';
								echo '</div>';
							}
						}
						return $one;
				...
				<script>
					function createUploader(){
						var uploader = new qq.FileUploader({
							...
							action: '../tools/valums-file-upload/server/ftp.php?dossier='+currentDir.substr(3),

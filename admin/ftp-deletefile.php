...
if(!admin()){
	...
	if(!is_file($target)) $errTab[]="L'élément donné ne semble pas être un fichier";
	// ne pas être un fichier sensible (htaccess)
	if($filename=='.htaccess') $errTab[]="Le fichier .htaccess ne peut pas être supprimé";
	if($filename=='index.php') $errTab[]="Le fichier index.php ne peut pas être supprimé";
	// ne pas être protégé (params.php)
	if(in_array(substr($target, 7), $p_ftp_proteges)) $errTab[]="Le fichier $filename est protégé contre la suppression";
	...
	if(sizeof($errTab)) echo '<div class="erreur"><ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
		...
		if($_GET['operation']=='delete'){
			if($_GET['lock']!='unlocked') $errTab[]="Erreur : fichier verrouillé.";
			else{
				if(!unlink($target))  $errTab[]="Erreur : suppression de $target echouée.";
			}
		...
		if(!$_POST['operation'] || $_POST['operation']=='delete' && sizeof($errTab)){

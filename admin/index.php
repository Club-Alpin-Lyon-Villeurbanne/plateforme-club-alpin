...
$errTab=array(); // gestion des erreurs
...
if($_POST['loginLocal']=='superadmin' && $_SERVER['HTTP_HOST'] == '127.0.0.1') superadmin_start();
if($_POST['loginLocal']=='hwc' && $_SERVER['HTTP_HOST'] == 'cafdemo.dev') superadmin_start();
if($_POST['loginLocal']=='admin' && $_SERVER['HTTP_HOST'] == '127.0.0.1') admin_start();
if($_POST['login']){
	...
	if(!sizeof($errTab)){
		if(stripslashes($_POST['login'])!=$login_admin)
			$errTab[]="Login ou mot de passe incorrect."; // bluff : c'est le login qui est faux. Ecrire le même message en cas d'erreur mdp
	}
...
<body style="text-align:center;">
	<?php
	if(sizeof($errTab)) echo '<div class="error"><ul><li>'.implode('</li><li>', $errTab).'</div><br />';
	if(!admin()){
		/* */

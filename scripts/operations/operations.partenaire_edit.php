<?php
	// PARAMS
	$allowedExts = array("png"); // we need transparency
	$uploaddir = 'ftp/partenaires/';

	$partenaireTab=array();
	$partenaireTab['part_id'] = intval(trim(stripslashes($_POST['part_id'])));
	$partenaireTab['part_order'] = intval(trim(stripslashes($_POST['part_order'])));
	$partenaireTab['part_type'] = intval(trim(stripslashes($_POST['part_type'])));
	$partenaireTab['part_enable'] = intval(trim(stripslashes($_POST['part_enable'])));
...
//	var_dump($partenaireTab);

	// verification du format des donnees
	if( !is_numeric($partenaireTab['part_id'])) $errTab[]="id partenaire invalide";
	if( !is_numeric($partenaireTab['part_order'])) $errTab[]="ordre affichage partenaire invalide";
	if( !is_numeric($partenaireTab['part_type'])) $errTab[]="type de partenaire invalide";
	if( !is_numeric($partenaireTab['part_enable'])) $errTab[]="statut partenaire invalide";

	if(strlen($partenaireTab['part_name'])<2) 		$errTab[]="Merci d'entrer un nom valide";
	if(strlen($partenaireTab['part_url'])<5) 		$errTab[]="Merci d'entrer une url valide";
	if(strlen($partenaireTab['part_desc'])<2) 		$errTab[]="Merci d'entrer une description valide";
...
//	var_dump($errTab);
	...
	if(!sizeof($errTab)){
		...
		if($partenaireTab['part_id'] == -1){
			...
			if(strcmp($partenaireTab['part_image'], $new_part_image) !== 0) {
...
//		error_log ($req);
		...
		if(!$mysqli->query($req)) $errTab[]="Erreur SQL : ".$mysqli->error;
			...
			if($partenaireTab['part_id'] > 0) $okTab[] = "Mise à jour du partenaire OK";
			else $okTab[] = "Création du partenaire OK";
...
*/
		if($_FILES['part_image']['size']>0){
			...
			if ((($_FILES["part_image"]["type"] == "image/png") && in_array($extension, $allowedExts))) {

<?php
	...
	$nomadMsg=array(); // message spécial par raport aux nomades

	// checks
	if(!strlen($msg)) $errTab[]="Veuillez entrer un message";
	if(!$id_evt) $errTab[]="ID invalide";
	...
	if($handle=$handleSql->fetch_array(MYSQLI_ASSOC)){
		...
		if(!allowed('evt_cancel', 'commission:'.$handle['code_commission']))	$errTab[]='Accès non autorisé';

		// Mise à jour : annulation
		if(!sizeof($errTab)){
			$req="UPDATE caf_evt SET cancelled_evt='1', cancelled_who_evt='".intval($_SESSION['user']['id_user'])."', cancelled_when_evt='".$p_time."'  WHERE caf_evt.id_evt =$id_evt";
		...
		if(!sizeof($errTab) && $handle['status_evt']==1){
			...
			$content_main="<h2>$subject</h2>
				<p>
					...
					vient d'être annulée par <a href=\"".$p_racine."voir-profil/".intval($_SESSION['user']['id_user']).".html\">".$_SESSION['user']['nickname_user']."</a>.
					Voici le message joint :
				</p>
				<p>&laquo;<i> ".nl2br(html_utf8($msg))." </i>&raquo;</p>
				<p><a href='".$p_racine."sortie/".html_utf8($handle['code_evt'])."-".intval($handle['id_evt']).".html' title=''>&lt; Voir la page dédiée</a></p>
				";
			$content_header="";
			$content_footer="";
			...
			if($handle['cycle_parent_evt']) $id_evt_forjoins=intval($handle['cycle_parent_evt']);
			else							$id_evt_forjoins=intval($handle['id_evt']);
			...
			if(!sizeof($errTab)){
				...
				$req.=")";
				if(!$mysqli->query($req)) $errTab[]="Erreur SQL (DELETE FROM caf_evt_join)";
			}
			...
			while($handle2=$handleSql2->fetch_array(MYSQLI_ASSOC)){
				...
				else	$nomadMsg[]=$handle2['civ_user'].' '.$handle2['firstname_user'].' '.$handle2['lastname_user'].' - '.$handle2['tel_user'].' - '.$handle2['tel2_user'];

<?php
	...
	if(!sizeof($errTab)){
		...
		$cout_transport 		= $mysqli->real_escape_string(floatval($cout_transport));
		...
		if ($newbus) {
			foreach ($newbus as $bus) {
				...
				if(!$mysqli->query($sql)) $errTab[]="Erreur SQL création du bus";
			}
		}

<?php
	...
	$status_evt=intval($_POST['status_evt']);

	// checks
	if(!$id_evt)					$errTab[]="Erreur d'identifiant";
	if(!allowed('evt_validate'))	$errTab[]="Vous ne semblez pas autorisé à effectuer cette opération";

	include SCRIPTS.'connect_mysqli.php';

	// save
	if(!sizeof($errTab)){
		$req="UPDATE caf_evt SET status_evt='$status_evt', status_who_evt=".intval($_SESSION['user']['id_user'])." WHERE caf_evt.id_evt =$id_evt";
		if(!$mysqli->query($req)) {
		...
		if(!$authorDatas) $errTab[]="User or evt not found";

	}

	// envoi de mail à l'auteur pour - lui confirmer la création / OU / l'informer du refus
	if(!sizeof($errTab) && ($status_evt==1 || $status_evt==2)){
		...
		if($status_evt==2){
			...
			$content_main="<h2>$subject</h2>
				...
				<p>&laquo;<i>".html_utf8(stripslashes($_POST['msg']?$_POST['msg']:'...'))."</i>&raquo;</p>
		...
		while($row=$result->fetch_assoc()){
			...
			$content_main="<h2>$subject</h2>
				...
				<p>
					...
					<a href=\"".$p_racine."voir-profil/".intval($authorDatas['id_user']).".html\">".$authorDatas['nickname_user']."</a>

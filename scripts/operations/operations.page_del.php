<?php
	...
	while($handle=$handleSql->fetch_array(MYSQLI_ASSOC)){
		...
		if(!getArrayFirstValue($handleSql2->fetch_array(MYSQLI_NUM))) 		$errTab[]="Il semble qu'aucune page ne soit autorisée à être supprimée ave cet id.";

		if(!sizeof($errTab)){

			// del page
			$req="DELETE FROM `".$pbd."page` WHERE `".$pbd."page`.`id_page` = $id_page LIMIT 1;";
			if(!$mysqli->query($req)) $errTab[]="Erreur BDD<br />".$req;
		...
		// LOG
		if(!sizeof($errTab))	mylog("page-delete", "Suppression de la page $code_page", false);
	}

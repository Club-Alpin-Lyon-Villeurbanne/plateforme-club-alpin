<?php
	...
	if(!$id_destination) $errTab[]="ID invalide";
...
include SCRIPTS.'connect_mysqli.php';;
    ...
    if(allowed('destination_supprimer')
        OR $destination['id_user_who_create'] == $_SESSION['user']['id_user']
    ...
    if (!sizeof($errTab)) {
        ...
        if(!$mysqli->query($req)) $errTab[]="Erreur SQL annulation destination";
    }
    ...
    if(!sizeof($errTab)){
        ...
        foreach ($sorties as $sortie) {
            ...
            $req="UPDATE ".$pbd."evt SET cancelled_evt='1', cancelled_who_evt='".intval($_SESSION['user']['id_user'])."', cancelled_when_evt='".$p_time."'  WHERE id_evt = ".$sortie['id_evt'];
		...
		if(!sizeof($errTab) && $destination['publie'] == 1){
			...
			$content_main="<h2>$subject</h2>
				<p>
					...
					viennent d'être annulées par <a href=\"".$p_racine."voir-profil/".intval($_SESSION['user']['id_user']).".html\">".$_SESSION['user']['nickname_user']."</a>.
					Voici le message joint :
				</p>
				<p>&laquo;<i> ".nl2br(html_utf8($msg))." </i>&raquo;</p>
				<p><a href='".$p_racine."destination/".html_utf8($destination['code'])."-".intval($destination['id']).".html' title=''>&lt; Voir la page dédiée</a></p>
				";
			$content_header="";
			$content_footer="";

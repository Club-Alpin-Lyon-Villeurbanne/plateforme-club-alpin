<?php
	...
	if(!sizeof($errTab)){
		...
		if($filiations){
			if(!sizeof($_POST['id_user_filiation'])) $errTab[]='Merci de choisir au moins une personne à inscrire';
			...
			foreach($_POST['id_user_filiation'] as $id_user_tmp){
				...
				if($id_user_tmp != $_SESSION['user']['id_user']){
					$req="SELECT COUNT(id_user) FROM ".$pbd."user WHERE cafnum_parent_user LIKE '".$mysqli->real_escape_string($_SESSION['user']['cafnum_user'])."' AND id_user=".intval($id_user_tmp);
					$result = $mysqli->query($req);
					$row = $result->fetch_row();
					if(!$row[0]) $errTab[]="ID '".intval($id_user_tmp)."' invalide pour l'inscription d'un adhérent affilié";
				}
			}
		}
        ...
        if ($is_destination) {
            ...
            $result = $mysqli->query($req);
            $row = $result->fetch_row();
            if($row[0]) $errTab[]="Cette destination ne semble pas publiée ou est annulée, les inscriptions sont impossible";
        }
		...
		$result = $mysqli->query($req);
		$row = $result->fetch_row();
		if($row[0]) $errTab[]="Cette sortie ne semble pas publiée, les inscriptions sont impossible";
		...
		// pour les inscriptions d'affilié
			...
			foreach($_POST['id_user_filiation'] as $id_user_tmp){
				$id_user_tmp=intval($id_user_tmp);
		...
		if(!sizeof($errTab)){
			...
			if(!$filiations){
                ...
                /* if (count(empietement_sortie($id_user, $evt)) > 0) {
                    ...
                    if(!$mysqli->query($req)) $errTab[]="Erreur SQL ".$req;
                // }
			}
			// filiations
			else{
		...
		if(!sizeof($errTab)){
			...
			."LIMIT 1; ";
			$result = $mysqli->query($req);
			while($row=$result->fetch_assoc()){	$destinataires[''.$row['id_user']] = $row;	}
			...
			."LIMIT 1 ";
			$result = $mysqli->query($req);
			while($row=$result->fetch_assoc())	$evt = $row;
		...
		if(!sizeof($errTab)){
			...
			if(!$filiations){
				...
				if(sizeof($inscrits) == 1 && $inscrits[0]['id_user'] == $_SESSION['user']['id_user']){
					$content_main="<h2>$subject</h2>
						<p>
							Bonjour $toName,<br />
							Vous venez de vous <i>".($is_destination?'inscrire':'préinscrire')."</i> à la sortie &laquo; <a href='$evtUrl'>$evtName</a> &raquo;.

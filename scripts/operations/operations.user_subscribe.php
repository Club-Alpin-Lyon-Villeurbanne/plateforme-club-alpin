<?php
	...
	if(strlen($lastname_user)<2) 	$errTab[]="Merci de renseigner un nom de famille valide";
	if(strlen($cafnum_user) != $limite_longeur_numero_adherent) 		$errTab[]="Merci de renseigner un numéro d'adhérent CAF valide ($limite_longeur_numero_adherent chiffres)";
	if(!isMail($email_user)) 		$errTab[]="Merci de renseigner une adresse e-mail valide";
	if(strlen($mdp_user)<6 or strlen($mdp_user)>12) 		$errTab[]="Le mot de passe doit faire de 6 à 12 caractères";
	...
	if(!sizeof($errTab)){
		...
		if(!sizeof($errTab)){
			...
			if($row[0]) $errTab[]="Le compte lié à ce numéro d'adhérent a été désactivé manuellement par un responsable. Nous vous invitons à contacter le Président, ou vice-Président du club pour en savoir plus.";
		}
		...
		if(!sizeof($errTab)){
			...
			$result = $mysqli->query($req);
			$row = $result->fetch_row();
			if($row[0]) $errTab[]="Ce numéro d'adhérent correspond déjà à une inscription sur le site. Si vous avez perdu vos identifiants, utilisez le lien <i>Mot de passe oublié</i> ci-contre à droite.";
		}
		...
		if(!sizeof($errTab)){
			...
			$result = $mysqli->query($req);
			$row = $result->fetch_row();
			if($row[0]) $errTab[]="Cette adresse e-mail correspond déjà à une inscription sur le site. Si vous avez perdu vos identifiants, utilisez le lien <i>Mot de passe oublié</i> ci-contre à droite.";
		}
		...
		if(!sizeof($errTab)){
			...
			$result = $mysqli->query($req);
			$row = $result->fetch_row();
			if(!$row[0]) $errTab[]="Désolé, nous ne trouvons pas ce numéro d'adhérent dans notre base de donnée. Si vous venez de vous (ré)inscrire au CAF, nous vons invitons à réessayer ultérieurement.";
		}
		...
		if(!sizeof($errTab)){
			...
			$result = $mysqli->query($req);
			$row = $result->fetch_row();
			if($row[0]) $errTab[]="La licence pour ce compte semble être expirée. Si vous venez de renouveler votre licence nous vous invitons à réessayer ultérieurement.";
		}
		...
		if(!sizeof($errTab)){
			...
			while($row = $result->fetch_assoc())
				$id_user=$row['id_user']; // ID : clé permettenat l'enregistrement ci-après

			if(!$id_user) $errTab[]="Le nom que vous avez entré ne correspond pas au numéro d'adhérent dans notre base de données.";
		}
		...
		if(!sizeof($errTab)){
			...
			while($row = $result->fetch_assoc()){
				...
				$nickname_user = str_replace(array(' ', '-', '\''), '', $firstname_user . substr(strtoupper($row['lastname_user']), 0, 1));
#				$nickname_user = str_replace(array(' ', '-', '\''), '', $row['firstname_user']) . substr($row['lastname_user'], 0, 1);
			}
			// secure SQL
			$nickname_user=$mysqli->real_escape_string($nickname_user);

			if(!$nickname_user) $errTab[]="Impossible de générer le pseudo. Merci de nous contacter.";
		}
		...
		if(!sizeof($errTab)){
			...
			if(!$mysqli->query($req)) $errTab[]="Erreur de sauvegarde";
		}

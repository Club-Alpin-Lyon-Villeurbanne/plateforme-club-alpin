...
*/
	...
	if(!isMail($userTab['email_user'])) 		$errTab[]="Adresse e-mail invalide";
	if(strlen($userTab['mdp_user'])>0 && (strlen($userTab['mdp_user'])<6 or strlen($userTab['mdp_user'])>12)) 		$errTab[]="Le mot de passe doit faire de 6 à 12 caractères";

	if( is_numeric($userTab['cafnum_user']!=1) || strlen($userTab['cafnum_user'])!=12) $errTab[]="Numéro d'adhérent invalide : ".$userTab['cafnum_user'];
	if( strlen($userTab['cafnum_user_new'])!=0 && ((!is_numeric($userTab['cafnum_user_new'])) || strlen($userTab['cafnum_user_new'])!=12)) $errTab[]="Nouveau numéro d'adhérent invalide : ".$userTab['cafnum_user_new'];
...
*/
		...
		if(!sizeof($errTab)){
			...
			else 	$okTab[] = "Mise à jour du compte";

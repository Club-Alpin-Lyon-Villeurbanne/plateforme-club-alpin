<?php
	...
	if(!user())	$errTab[]="Seuls les adhérents connectés peuvent commenter pour le moment";
	if($_POST['unlock1'] != 'unlocked') $errTab[]="L'antispam n'a pas autorisé l'envoi. Merci de cliquer sur le bouton &laquo;OK&raquo; pour envoyer le message.";
	...
	$parent_comment=intval($_POST['parent_comment']);

	// checks
	if(strlen($cont_comment) < 10) 	$errTab[]="Par soucis de pertinence, les commentaires doivent être supérieurs à 10 caractères.";
	if(!$parent_type_comment)		$errTab[]="Parent type non défini.";
	if(!$parent_comment) 			$errTab[]="Parent non défini.";
	...
	if(!sizeof($errTab)){
		...
		$req="INSERT INTO caf_comment(id_comment, status_comment, tsp_comment, user_comment, name_comment, email_comment, cont_comment, parent_type_comment, parent_comment)
								VALUES (NULL ,  '1', 			 '$p_time',  '".intval($_SESSION['user']['id_user'])."',  '',  '',  '$cont_comment_mysql',  '$parent_type_comment',  '$parent_comment');";

<?php

	// Utilisateur connecté ?
	if(!user()) $errTab[]="Vous avez été déconnecté. L'opération n'a pas été effectuée.";

	// mise à jour des informations de niveau
    // il devrait y avoir une vérification de l'autorisation à modifier le niveau utilisateur
    
	if(!sizeof($errTab)){
        include SCRIPTS.'connect_mysqli.php';
        
        if (isset($_POST['new_niveau']) && is_array($_POST['new_niveau'])) {
            foreach ($_POST['new_niveau'] as $niveau) {
                
                $id_user =intval($niveau['id_user']);
                $id_commission =intval($niveau['id_commission']);
                $niveau_technique=intval($niveau['niveau_technique']);
                $niveau_physique=intval($niveau['niveau_physique']);
                $commentaire=$mysqli->real_escape_string(trim(stripslashes($niveau['commentaire'])));
                if (empty($commentaire)) $commentaire = NULL;
                
                if (!$errTab && (!is_null($commentaire) || $niveau_technique > 0 || $niveau_physique > 0)) {
                    $req = "INSERT INTO `".$pbd."user_niveau` (`id`, `id_user`, `id_commission`, `niveau_technique`, `niveau_physique`, `commentaire`) VALUES (NULL, '".$id_user."', '".$id_commission."', '".$niveau_technique."', '".$niveau_physique."', ";
                    if (is_null($commentaire)) $req .= 'NULL'; 
                    else $req .= "'".$commentaire. "' ";
                    $req .= ");";
                    if(!$mysqli->query($req)) 
                        $errTab[]="Erreur SQL lors Insertion note pour commission ".$id_commission." et utilisateur ".$id_user;
                }
            }
        }
        
        if (isset($_POST['niveau']) && is_array($_POST['niveau'])) {
            foreach ($_POST['niveau'] as $id_niveau => $niveau) {
                
                $id=trim(stripslashes($niveau['id']));
                if ($id_niveau != $id) $errTab[] = 'Id niveau ne correspond pas';
                
                $niveau_technique=intval($niveau['niveau_technique']);
                $niveau_physique=intval($niveau['niveau_physique']);
                $commentaire=$mysqli->real_escape_string(trim(stripslashes($niveau['commentaire'])));
                if (empty($commentaire)) $commentaire = NULL;
                
                if (!$errTab) {
                    $req = "UPDATE `".$pbd."user_niveau` SET `niveau_technique` = '".$niveau_technique."', `niveau_physique` = '".$niveau_physique."', `commentaire` = ";
                    if (is_null($commentaire)) $req .= 'NULL'; 
                    else $req .= "'".$commentaire. "' ";
                    $req .= " WHERE `".$pbd."user_niveau`.`id` = ".$id.";";
                    if(!$mysqli->query($req)) 
                        $errTab[]="Erreur SQL";
                }
            }
        }
        
        $mysqli->close();
    }
<?php
	...
	$status_legal_evt=intval($_POST['status_legal_evt']);

	// checks
	if(!$id_evt)						$errTab[]="Erreur d'identifiant";
	if(!allowed('evt_legal_accept'))	$errTab[]="Vous ne semblez pas autorisé à effectuer cette opération";

	include SCRIPTS.'connect_mysqli.php';

	// save
	if(!sizeof($errTab)){
		$req="UPDATE caf_evt SET status_legal_evt='$status_legal_evt', status_legal_who_evt=".intval($_SESSION['user']['id_user'])." WHERE caf_evt.id_evt =$id_evt";
		if(!$mysqli->query($req)) {
	...
	if(!sizeof($errTab) && ($status_legal_evt==1 || $status_legal_evt==2)){

		// content vars
		if($status_legal_evt==1){
	...
	if(!sizeof($errTab) && $do_mail_evt_legal_update){

<?php
	...
	$priority_page=intval($_POST['priority_page']) /10;

	// meta description par defaut, ou sur-mesure ?
	if($default_description_page) 	$meta_description_page=1; // sur mesure
	else 							$meta_description_page=0; // par defaut (celle du site)
	...
	//
	$req="SELECT COUNT(id_page) FROM `".$pbd."page` WHERE `code_page` LIKE '$code_page' LIMIT 1";
	$handleSql=$mysqli->query($req);
	if(getArrayFirstValue($handleSql->fetch_array(MYSQLI_NUM))) 		$errTab[]="Ce code est déjà utilisé par une autre page, veuillez le modifier pour le rendre unique.";
	...
	if(!sizeof($errTab)){
		...
		if(!$mysqli->query($req)) $errTab[]="Erreur BDD title";
	}
	// save description // si necessaire
	if(!sizeof($errTab) && $default_description_page){
		$lang_content_inline=$p_langs[0];
	...
	$mysqli->close();
	// LOG
	if(!sizeof($errTab))	mylog("page-libre-create", "Création de la page libre $default_name_page ($code_page)");
?>
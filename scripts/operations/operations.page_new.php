<?php
	...
	$parent_page=intval($_POST['parent_page']);			// id parent, ou 0
	$vis_page=0; // par défaut

	if(!$titre_page) 	$erreur="Merci d'entrer un titre";
	if(!$nom_page) 	$erreur="Merci d'entrer un nom de page";
	...
	if(!$erreur){
		...
		for($i=1; $doublon; $i++){
			...
			if(!getArrayFirstValue($countSql->fetch_array(MYSQLI_NUM)))	$doublon=false;
		}

<?php
	...
	if(strlen($civ_user)<1) 		$errTab[]="Merci d'entrer la civilité";
	if(strlen($firstname_user)<2) 	$errTab[]="Merci d'entrer un prenom valide";
	if(strlen($lastname_user)<2) 	$errTab[]="Merci d'entrer un nom de famille valide";
	...
	if(!isMail($email_user)) 		$errTab[]="Adresse e-mail invalide";
	if(strlen($mdp_user)<6 or strlen($mdp_user)>12) 		$errTab[]="Le mot de passe doit faire de 6 à 12 caractères";
	if($mdp_user != $_POST['mdp_user_confirm']) 			$errTab[]="Veuillez entrer deux fois le même mot de passe, sans espace";
	// date de naissance :
	if(!preg_match("#[0-9]{2}/[0-9]{2}/[0-9]{4}#", $birthday_user)) $errTab[]="La date de naissance doit être au format jj/mm/aaaa.";
	...
	if(!sizeof($errTab)){
		...
		if($row[0]) $errTab[]="Un compte validé existe déjà avec cette adresse e-mail. Avez-vous <a href=\"includer.php?p=pages/mot-de-passe-perdu.php\" class=\"fancyframe\" title=\"\">oublié le mot de passe ?</a>";
		else{

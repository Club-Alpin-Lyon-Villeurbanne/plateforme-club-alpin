<?php
	...
	if(!is_array($_POST['id_user']))	$errTab[]="Tableau des ID manquant";
	else{
		...
		if(sizeof($_POST['id_user']) != sizeof($_POST['role_evt_join'])) $errTab[]="Erreur de correspondance de la var role_evt_join";
	}
	...
	if(!sizeof($errTab)){
        ...
        if ($_POST['id_destination']) {
            ...
            if ($inscriptions_status['status'] != true) $errTab[]="Les inscriptions à la destination ne sont pas possibles";
        }
		...
		foreach ($_POST['id_user'] as $i => $user){
			...
			if(!sizeof($errTab)){
				...
				if(!isMail($toMail)) $errTabMail[]="Les coordonnées du contact sont erronées (l'inscription est réalisée quand même)";
				...
				if(!sizeof($errTabMail)){
					...
					$content_main="<h2>$subject</h2>
						<p>
							Bonjour $toName,<br />
							Vous venez d'être inscrit ".($role_evt_join=='manuel'?'':' en tant que <i>'.$role_evt_join.'</i>')."
					...
					if(!$mail->Send()){
						$errTabMail[]="Échec à l'envoi du mail à ".html_utf8($handle2['email_user']).". Plus d'infos : ".($mail->ErrorInfo);
					}
				}
			}
		}
    ...
    if (!sizeof($errTab)) {$_POST['id_user'] = array(0); $_POST['result'] = 'success';}
	$mysqli->close;

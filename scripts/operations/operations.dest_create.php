<?php
	...
	if(!sizeof($errTab)){
		...
		if(!$mysqli->query($sql)) $errTab[]="Erreur SQL lors de la création du lieu";
            ...
            if(!$mysqli->query($sql)) $errTab[]="Erreur SQL lors de la création de la destination : ";
			else{
...
if (!sizeof($errTab)) header('Location:'.$p_racine.'creer-une-sortie/creer-une-destination/update-'.$id_dest_to_update.'.html');

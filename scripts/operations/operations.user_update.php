<?php
	...
	// check user online
	if(!user()) $errTab[]="Vous avez été déconnecté. L'opération n'a pas été effectuée.";
	...
	if(!sizeof($errTab)){
		...
		if(!$id_user) $errTab[]="Erreur technique : ID manquant";
		if(!isMail($email_user_mailchange) && strlen($email_user_mailchange)>0) $errTab[]="Vous avez demandé à remplacer votre adresse e-mail, mais elle semble invalide.";
		...
		if(!sizeof($errTab)){
			...
			else{ user_login($_SESSION['user']['email_user'], false); }
	...
	if(!sizeof($errTab) && $_FILES['photo']['size']>0){
		// CHECKS
		...
		if ((($_FILES["photo"]["type"] == "image/jpeg")
			...
			if ($_FILES["photo"]["error"] > 0){
				...
				$i=1;
				while(file_exists($uploaddir.$i.'-profil.jpg')) $i++;
				...
				if(move_uploaded_file($_FILES['photo']['tmp_name'], $uploaddir.$filename)){
					...
					if(!file_exists($rep_Dst) && $id_user) mkdir($rep_Dst);
					...
					// <= crop
					// >= redim
					if($size[0]/$size[1] <= 55/55){ 	$W_fin=55;	$H_fin=0;	}
					else{								$W_fin=0;	$H_fin=55;	}
					// redim
					if(!fctredimimage($W_fin, $H_fin, $rep_Dst, $img_Dst, $rep_Src, $img_Src)) {
				...
				else $errTab[]="Erreur lors du déplacement du fichier";
			}
		}
		else $errTab[]="La photo doit être au format .jpg et peser moins de 5Mo !";
	}
	...
	if(strlen($email_user_mailchange) > 0){
		...
		if(getArrayFirstValue($handleSql->fetch_array(MYSQLI_NUM)) > 0) $errTab[] = "Votre demande de modification d'e-mail est refusée : Un compte existe déjà avec cette adresse e-mail.";
		...
		if(!sizeof($errTab)){
			...
			if(!$mysqli->query($req)) $errTab[]="Erreur SQL";
			else $id_user_mailchange = $mysqli->insert_id;
		}

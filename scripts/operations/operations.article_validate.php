<?php
	...
	$status_article=intval($_POST['status_article']);

	// checks
	if(!$id_article)					$errTab[]="Erreur d'identifiant";
	if(!allowed('article_validate'))	$errTab[]="Vous ne semblez pas autorisé à effectuer cette opération";
	...
	if(!sizeof($errTab)){
		...
		$req="UPDATE caf_article SET status_article='$status_article', status_who_article=".intval($_SESSION['user']['id_user'])." WHERE caf_article.id_article =$id_article";
		if(!$mysqli->query($req)) {
		...
		if(!$authorDatas) $errTab[]="User or article not found";
	...
	if(!sizeof($errTab) && ($status_article==1 || $status_article==2)){

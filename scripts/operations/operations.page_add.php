<?php
	...
	$parent_page=intval($_POST['parent_page']);
	$menu_page=intval($_POST['menu_page']);

	$titreTab=$_POST['titre'];
	for($i=0; $i<sizeof($p_langs); $i++){
		if(!strlen($titreTab[$i]))		$errTab[]="Nom de la page trop court pour cette langue : ".strtoupper($p_langs[$i]);
	}
	$menuTab=$_POST['menuname'];
	if($menu_page){
		for($i=0; $i<sizeof($p_langs); $i++){
			if(!strlen($menuTab[$i]))		$errTab[]="Intitulé dans le menu trop court pour cette langue : ".strtoupper($p_langs[$i]);
		}
	}
	...
	$meta_title_page=intval($_POST['meta_title_page']);
	$priority_page=intval($_POST['priority_page']) /100;
	...
	// checks
	$pattern = "#^[a-z0-9-]+$#";
	if(!strlen($code_page))						$errTab[]="Code de page trop court";
	if(!preg_match($pattern, $code_page))	$errTab[]="Le code de la page ne respecte pas le format demandé : chiffres, lettres sans accents, et tirets.";
	...
	if(getArrayFirstValue($handleSql->fetch_array(MYSQLI_NUM))) 		$errTab[]="Ce code est déjà utilisé, veuillez en entrer un autre";
	...
	if(!sizeof($errTab)){
		...
		if(!$mysqli->query($req)) $errTab[]="Erreur BDD 1";
		else{
			...
			if(!$mysqli->query($req)) $errTab[]="Erreur BDD 2";
		}
	}
	// save titles
	if(!sizeof($errTab)){
		for($i=0; $i<sizeof($p_langs); $i++){
			...
			if(!$mysqli->query($req)) $errTab[]="Erreur BDD titles";

		}
	}
	// save menu
	if(!sizeof($errTab) && $menu_page){
		for($i=0; $i<sizeof($p_langs); $i++){
			$lang_content_inline=$p_langs[$i];
			$contenu_content_inline=$mysqli->real_escape_string(stripslashes($titreTab[$i]));
			$req="INSERT INTO `".$pbd."content_inline` (`id_content_inline` ,`groupe_content_inline` ,`code_content_inline` ,`lang_content_inline` ,`contenu_content_inline` ,`date_content_inline` ,`linkedtopage_content_inline`)
												VALUES (NULL , '4', 'mainmenu-$code_page', '$lang_content_inline', '$contenu_content_inline', '$p_time', '');";
			if(!$mysqli->query($req)) $errTab[]="Erreur BDD titles";
	...
	$mysqli->close();
	// LOG
	if(!sizeof($errTab))	mylog("page-create", "Création de la page $default_name_page ($code_page)");

...
if(user()){
	...
	elseif(!$id_commission){
		...
		if(!$commission) echo '<p class="erreur">Commission introuvable</p>';
		else{

			// redirection si OK
			if(($_POST['operation'] == 'commission_majvis' || $_POST['operation'] == 'commission_majvis') && !sizeof($errTab)){
				?>
			...
			if(!$commission['vis_commission']){
				...
				<form action="<?php echo $versCettePage;?>" method="post" enctype="multipart/form-data" class="loading">
					...
					if($_POST['operation'] == 'commission_majvis' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
					?>
					<br />
					<br />

...
<?php /* Paiement en ligne */ if ($evt['cb_evt']) { ?>
    ...
    <p>
        <label style="width:100%;clear:both;overflow:hidden;">
            <input type="checkbox" name="jeveuxpayerenligne" onclick="$('.paiement').slideToggle(200)"  <?php echo (($my_choices['is_cb'] == 1) ? ' checked="checked" ':'') ; ?> />
            Cochez cette case si vous souhaitez payer en ligne
            <div class="paiement" style="display:<?php if($my_choices['is_cb'] == 1) echo 'block'; else echo 'none'; ?>">
    ...
    <?php foreach($filiations as $enfant){ 
        ...
        <label for="filiation_id_user_<?php echo intval($enfant['id_user']); ?>" style="width:100%;clear:both;overflow:hidden;">
            <input
                ...
                onclick="<?php echo "$('#paiement_enfant_".intval($enfant['id_user'])."').slideToggle(200)"; ?>"
                class="custom"
                name="id_user_filiation[]"
                ...
                <?php if ($evt['cb_evt'] == '1') { ?>
                <div id="<?php echo "paiement_enfant_".intval($enfant['id_user']); ?>" style="display:<?php if($my_choices['is_cb'] == 1 ) echo 'hidden'; else echo 'none'; ?>">
                <a style="border:1px solid red; background:rgba(255,153,153,.4);"
                    href="<?php echo $p_url_paiement ?>
?lck_vads_order_id=<?php echo rawurlencode(html_utf8($new_title)); ?>
&lck_vads_ext_info_Encadrant=<?php echo rawurlencode(html_utf8($encadrant_name)); ?>

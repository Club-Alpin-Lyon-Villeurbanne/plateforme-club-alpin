...
$style = null;
if ( is_null($my_choices['id_bus_lieu_destination']) && is_null($my_choices['id_destination']) && is_null($my_choices['is_covoiturage']) ) {
    ...
    <div style="text-align:center; padding:0 30px 0 0">
        ...
        <a class="biglink" href="javascript:void(0)" onclick="$('#inscription').slideToggle(200)" title="" <?php echo $style; ?> >
            ...
            <?php } else { ?>
                JE METS A JOUR MES PREFERENCES <?php if (!is_null($style)) { ?><img src="img/base/bullet_error.png" title="Mettre à jour les préférences" width="16px">&nbsp;<?php } ?>
            <?php } ?>
            <?php
            // incriptions de filiation ?
            if(sizeof($filiations)) echo '<span class="bleucaf">&nbsp; / &nbsp;</span> INSCRIRE DES ADHÉRENTS AFFILIÉS';
            ?>
        </a>
    </div>
<?php

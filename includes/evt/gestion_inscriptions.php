...
if($evt['cancelled_evt']!='1'){
    if(
        ...
        || $monStatut=='encadrant'
        || $monStatut=='coencadrant'
        || ($_SESSION['user']['status'] && in_array('Salarié', $_SESSION['user']['status']))
        || ((allowed('evt_join_notme')||allowed('evt_unjoin_notme')||allowed('evt_joining_accept')||allowed('evt_joining_refuse')) && ($_SESSION['user']['status'] && in_array('Resp. de commission, '.$evt['code_commission'], $_SESSION['user']['status'])))
        )
        {
        // droit de modification en fonction des conditions ci-dessus :
        $droitDeModif = false;
        // je suis salarie
        if ($_SESSION['user']['status'] && in_array('Salarié', $_SESSION['user']['status']))
            $droitDeModif=true;
        ...
        if(admin() || allowed('evt_join_doall'))
            $droitDeModif=true;
        // ou je participe à cette sortie en tant qu'encadrant / coencadrant
        if($monStatut=='encadrant' || $monStatut=='coencadrant')
            $droitDeModif=true;
        // ou je suis resp de comm pour la sortie
        if ((allowed('evt_join_notme')||allowed('evt_unjoin_notme')||allowed('evt_joining_accept')||allowed('evt_joining_refuse')) && ($_SESSION['user']['status'] && in_array('Resp. de commission, '.$evt['code_commission'], $_SESSION['user']['status'])))
            $droitDeModif=true;
        ...
        <div id="inscription-gestion">
            ...
            <form action="<?php echo $versCettePage;?>#inscription-gestion" method="post" class="loading">
                ...
                <?php
                // TABLEAU erreurs
                if($_POST['operation'] == 'user_join_update_status' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
                if($_POST['operation'] == 'user_join_update_status' && !sizeof($errTab))	echo '<p class="info">Mise à jour effectuée à '.date("H:i", $p_time).'.</p><br />';

                // TABLEAU alertes
                if($_POST['operation'] == 'user_join_update_status' && sizeof($addAlert))	echo '<div class="alerte">Attention : <ul><li>'.implode('</li><li>', $addAlert).'</li></ul></div><br />';
                ...
                <table class="datatable">
                    <thead>
                        <tr>
                            ...
                            <th width='15%'>Rôle</th>
                            <th width='10%'>Date</th>
                            <?php if ($evt['cb_evt'] == 1 && $evt['repas_restaurant'] == 1) { ?>
							<th width='20%'>Contact</th>
							<?php } elseif (($evt['cb_evt'] == 1 && $evt['repas_restaurant'] == 0) || ($evt['cb_evt'] == 0 && $evt['repas_restaurant'] == 1)) { ?>
                    ...
                    <tbody>
                        ...
                        while($row=$result->fetch_assoc()){
                            ...
                            // STATS
                            $nDemandes++;
                            if($row['status_evt_join']==1)	{
                                $nAcceptees++;
                                if($row['is_cb']==1)	$nCb++;
                                if(is_null($row['is_cb']))	$nCb_nsp++;
                                if($row['is_restaurant']==1)	$nRestaurant++;
                                if(is_null($row['is_restaurant']))	$nRestaurant_nsp++;
                            }
                            if($row['status_evt_join']==2)	$nRefusees++;
                            if($row['status_evt_join']==3)	$nAbsents++;
                            ...
                            echo '<tr class="status'.($row['status_evt_join']).'" style="color:gray; font-size:10px;">';
                                    ...
                                    echo '<td class="mini joinlabels status'.($row['status_evt_join']).'">
                                        ...
                                        if($row['role_evt_join'] != 'encadrant'){// && $row['role_evt_join'] != 'coencadrant'
                                            if($droitDeModif){
                                                echo '<span style="display:none">'.intval($row['status_evt_join']).'</span>
                                                <input type="hidden" name="id_evt_join[]" value="'.intval($row['id_evt_join']).'" />';

                                                if($row['status_evt_join']==0){
                                                    // EN ATTENTE
                                                    echo '<label for="join_'.intval($row['id_evt_join']).'_0">
                                                        <input '.($row['status_evt_join']==0 ? 'checked="checked"':'').' '.($disable0 ? 'disabled="disabled"':'').' type="radio" name="status_evt_join_'.intval($row['id_evt_join']).'" id="join_'.intval($row['id_evt_join']).'_0" value="0" />
                                                        En attente
                                                    </label>';
                                                }

                                                if (($p_time >= $evt['tsp_end_evt'] AND $row['status_evt_join']==1) || $p_time < $evt['tsp_end_evt'])
                                                echo '<label for="join_'.intval($row['id_evt_join']).'_1">
                                                <input '.($row['status_evt_join']==1 ? 'checked="checked"':'').' '.($disable1 ? 'disabled="disabled"':'').' type="radio" name="status_evt_join_'.intval($row['id_evt_join']).'" id="join_'.intval($row['id_evt_join']).'_1" value="1" />
                                                Accepté
                                                </label>';

                                                if ($p_time >= $evt['tsp_end_evt'] AND $row['status_evt_join']!=2)
                                                    echo '<label for="join_'.intval($row['id_evt_join']).'_3">
                                                        <input '.($row['status_evt_join']==3 ? 'checked="checked"':'').' '.($disable3 ? 'disabled="disabled"':'').' type="radio" name="status_evt_join_'.intval($row['id_evt_join']).'" id="join_'.intval($row['id_evt_join']).'_3" value="3" />
                                                        Absent
                                                    </label>';

                                                if (($p_time >= $evt['tsp_end_evt'] AND $row['status_evt_join']==2) || $p_time < $evt['tsp_end_evt'])
                                                echo '<label for="join_'.intval($row['id_evt_join']).'_2">
                                                <input '.($row['status_evt_join']==2 ? 'checked="checked"':'').' '.($disable2 ? 'disabled="disabled"':'').' type="radio" name="status_evt_join_'.intval($row['id_evt_join']).'" id="join_'.intval($row['id_evt_join']).'_2" value="2" />
                                                Refusé
                                                </label>';

                                                if ($p_time < $evt['tsp_end_evt'])
                                                echo '<label for="join_'.intval($row['id_evt_join']).'_-1">
                                                <input name="status_evt_join_'.intval($row['id_evt_join']).'" '.($disable_1 ? 'disabled="disabled"':'').' type="radio" id="join_'.intval($row['id_evt_join']).'_-1" value="-1" />
                                                Désinscrire
                                                </label>';

                                            } else{
                                                //echo '<input type="hidden" name="status_evt_join_'.intval($row['id_evt_join']).'" value="'.intval($row['status_evt_join']).'" />';
                                                echo ''.($row['status_evt_join']==0 ? 'En attente':'')
                                                .($row['status_evt_join']==1 ? 'Accepté':'')
                                                .($row['status_evt_join']==2 ? 'Refusé':'')
                                                .($row['status_evt_join']==3 ? 'Absent':'')
                                                .($row['status_evt_join']==-1 ? 'Désinscrire':'');
                                    ...
                                    echo '</td><td class="mini" > ';
                                        ...
                                        } elseif($droitDeModif && (strcmp($row['role_evt_join'],'manuel')==0 || strcmp($row['role_evt_join'],'inscrit')==0 || strcmp($row['role_evt_join'],'benevole')==0)){
                                            if(strcmp($row['role_evt_join'],'manuel')==0){
                                                //manuel
                                                echo '<label style="display:block; white-space:nowrap;" for="role_join_'.intval($row['id_evt_join']).'_m">
                                                    <input  '.(strcmp($row['role_evt_join'],'manuel')==0 ? 'checked="checked"':'').' name="role_evt_join_'.intval($row['id_evt_join']).'" type="radio" id="role_join_'.intval($row['id_evt_join']).'_m" value="manuel" />
                                                    Manuel
                                                    </label>';
                                            }
                                            echo '<label style="display:block; white-space:nowrap;" for="role_join_'.intval($row['id_evt_join']).'_i">
                                                <input  '.(strcmp($row['role_evt_join'],'inscrit')==0 ? 'checked="checked"':'').' name="role_evt_join_'.intval($row['id_evt_join']).'" type="radio" id="role_join_'.intval($row['id_evt_join']).'_i" value="inscrit" />
                                                Inscrit
                                                </label>';
                                            echo '<label style="display:block; white-space:nowrap;" for="role_join_'.intval($row['id_evt_join']).'_b">
                                                <input '.(strcmp($row['role_evt_join'],'benevole')==0 ? 'checked="checked"':'').' name="role_evt_join_'.intval($row['id_evt_join']).'" type="radio" id="role_join_'.intval($row['id_evt_join']).'_b" value="benevole" />
                                                Bénévole
                                                </label>';
                                    ...
                                    if ($evt['cb_evt'] == 1) {
                                        echo '<td><img src="img/base/'.($row['is_cb'] == '1' ? 'cb-oui.png' : ($row['is_cb'] == '0' ? 'cb-non.png' :'cb-nsp.png')).'" title="'.($row['is_cb'] == '1' ? 'Oui' : ($row['is_cb'] == '0' ? 'Non' :'NSP')).'" /></td>';
                                    }
                ...
                if($droitDeModif){
                    ?>
                    <p class="mini">
                        <input type="checkbox" name="disablemails" <?php if($_POST['disablemails']=='on') echo 'checked="checked"';?> />
                ...
                else echo '<p class="mini">'.$monStatut.' : Vous ne pouvez pas modifier les inscriptions</p>';
                ?>
                <br />
                <br />
            </form>
        ...
        ?>
                    ...
                    <form action="<?php echo $versCettePage;?>#autresoptions" method="post" id="contact-inscrits" style="display: <?php if($_POST['operation'] != 'evt_user_contact') echo 'none'; ?>; border : 1px solid #eaeaea; box-shadow:0 0 20px -10px gray;; padding:10px; margin:10px 0 20px 0;; border-radius:10px">
                        <input type="hidden" name="operation" value="evt_user_contact" />
                        <input type="hidden" name="id_evt" value="<?php echo intval($evt['id_evt']);?>" />

                        <h2>Formulaire de contact</h2>
                        <?php
                        // MESSAGES A LA SOUMISSION
                        if($_POST['operation'] == 'evt_user_contact' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
                        if($_POST['operation'] == 'evt_user_contact' && !sizeof($errTab))	echo '<p class="info">Votre message a bien été envoyé.</p>';
                        ?>

                        <input type="radio" name="status_sendmail" id="status_sendmail_1" value="1" <?php if($_POST['operation'] != 'evt_user_contact' || $_POST['status_sendmail']=='1') echo 'checked="checked"';?> /><label for="status_sendmail_1"> Envoyer uniquement aux adhérents confirmés pour la sortie</label><br />
                        <input type="radio" name="status_sendmail" id="status_sendmail_2" value="2" <?php if($_POST['status_sendmail']=='2') echo 'checked="checked"';?> /><label for="status_sendmail_2"> Envoyer uniquement aux adhérents refusés/absents</label><br />
                        <input type="radio" name="status_sendmail" id="status_sendmail_0" value="0" <?php if($_POST['status_sendmail']=='0') echo 'checked="checked"';?> /><label for="status_sendmail_0"> Envoyer uniquement aux adhérents en attente de confirmation</label><br />
                        <input type="radio" name="status_sendmail" id="status_sendmail_all" value="*" <?php if($_POST['status_sendmail']=='*') echo 'checked="checked"';?> /><label for="status_sendmail_all"> Envoyer à toute la liste des inscrits, confirmés ou non</label><br />

...
if(admin() ||
    (
        ...
        (
            ...
            $monStatut=='encadrant'	|| $monStatut=='coencadrant' ||
            allowed('evt_validate', 'commission:'.$evt['code_commission'])
        ) ||
        (
            $_SESSION['user']['status'] &&
            in_array('Salarié', $_SESSION['user']['status'])
        ) ||
        (
            (
                ...
                $_SESSION['user']['status'] &&
                in_array('Resp. de commission, '.$evt['code_commission'], $_SESSION['user']['status'])
            )
        )
    )
) {
...
$nAccepteesCalc = sizeof($evt['joins']['encadrant'])+sizeof($evt['joins']['coencadrant'])+sizeof($evt['joins']['benevole'])+sizeof($evt['joins']['inscrit'])+sizeof($evt['joins']['manuel']);
    ...
    <tr>
        ...
        <td style="border:0">
            <table style='width:560px'>
                <thead>
                <tr>
                    <th colspan="3" style="text-align:center; font-size:17px"><small>FEUILLE DE SORTIE</small><br><?php echo html_utf8($evt['titre_evt']); ?><?php if ($destination) echo '<br><small>Destination : ['.$destination['nom'].']</small>'; ?></th>
                </tr>
                </thead>
                <tbody>
                <?php if ($evt['status_legal_evt'] == 1) { ?>
...
<table>
    ...
    <tr>
        ...
        <td width='30%'><?php echo html_utf8($evt['titre_evt']); ?><?php if (count($evt['groupe'])>0) { echo ' - '.$evt['groupe']['nom']; } ?></td>
    </tr>
    <tr>
        <th>DISTANCE : </th>
...
<?php if ($destination) { $b = 1; echo '<ul><img src="img/bus.png" title="bus" class="lft mr10" />'; foreach ($destination['bus'] as $id_bus => $bus) {
    ...
    }echo '</ul></li>'; } echo '</ul><br class="clear"> ';  } ?>
...
<table>
    ...
    // lignes vides
    if (!isset($_GET['hide_blank']) || $_GET['hide_blank'] != 'y') {

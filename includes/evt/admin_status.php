<?php
	...
	if($evt['status_evt']=='0') {
		...
		if ((allowed('evt_validate_all') || allowed('evt_validate', 'commission:'.$evt['code_commission']) )&& ( $evt['status_evt']!=1 || ($evt['status_evt']==1 && $evt['cancelled_evt']==1) ) ) {
			...
			echo '<input type="hidden" name="id_evt" value="'.(intval($evt['id_evt'])).'" />';
			...
			echo '<input type="button" value="Refuser" class="nice2 red" onclick="$.fancybox($(this).next().html())" title="Ne pas autoriser la publication de cette sortie. Vous devrez ajouter un message au créateur de la sortie." />
				<div style="display:none" id="refuser-'.intval($evt['id_evt']).'">
					<form action="'.$versCettePage.'?forceshow=true" method="post" class="loading">
						...
						<input type="hidden" name="id_evt" value="'.(intval($evt['id_evt'])).'" />
        ...
        if ($evt['cancelled_who_evt'] && $evt['cancelled_who_evt']['id_user']) echo ', par '.userlink($evt['cancelled_who_evt']['id_user'], $evt['cancelled_who_evt']['nickname_user']);
        else echo ', suite à annulation de la destination.';
        echo '<br />';
	...
	} elseif($evt['status_evt']=='1' && ( allowed('evt_validate_all') || allowed('evt_validate', 'commission:'.$evt['code_commission'])  ) ) {
		// evt publie
		$messageDiv=true;
	...
	if(($evt['user_evt'] == $_SESSION['user']['id_user']) || allowed('evt_validate_all') || allowed('evt_validate', 'commission:'.$evt['code_commission'])){
		...
		if ($evt['tsp_end_evt'] > $p_time) {
			if ( allowed('evt_delete', 'commission:'.$evt['code_commission']) && ( $evt['status_evt']!=1 || ($evt['status_evt']==1 && $evt['cancelled_evt']==1) )) {
				// supprimer
				echo '<a class="nice2 noprint red" href="supprimer-une-sortie/'.html_utf8($evt['code_evt']).'-'.intval($evt['id_evt']).'.html" title="Supprimer définitivement la sortie ci-dessous"><img src="img/base/x2.png" alt="" title="" style="" />&nbsp;&nbsp;Supprimer cette sortie</a>';
			} elseif(allowed('evt_cancel', 'commission:'.$evt['code_commission']) && $evt['cancelled_evt']!='1') {
				//annuler
				echo '<a class="nice2 noprint red" href="annuler-une-sortie/'.html_utf8($evt['code_evt']).'-'.intval($evt['id_evt']).'.html" title="Annuler la sortie ci-dessous">

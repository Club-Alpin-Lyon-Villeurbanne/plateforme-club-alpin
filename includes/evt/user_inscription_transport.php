<div class="radio-nice ">
                        ...
                        <?php foreach ($bus['ramassage'] as $point) { ?>
                            <label>
                                <?php if ($bus['places_disponibles'] > 0) { ?>
                                    <input type="radio" name="id_bus_lieu_destination" value="<?php echo $point['bdl_id']; ?>" <?php if ($_POST['id_bus_lieu_destination']==$point['bdl_id'] || $my_choices['id_bus_lieu_destination']==$point['bdl_id']){echo ' checked="checked" ';} ?> >
                                    <?php $all_bus_plein = false; ?>
                                <?php } else { ?>
                                    <img src="img/base/cross.png">
                                    <?php if (!isset($all_bus_plein)) $all_bus_plein = true; ?>
                                <?php } ?>
    ...
    <label>
        <input type="radio" name="id_bus_lieu_destination" value="-1" <?php if ($_POST['id_bus_lieu_destination']==-1 || $my_choices['is_covoiturage'] == 1){echo ' checked="checked" ';} ?> >

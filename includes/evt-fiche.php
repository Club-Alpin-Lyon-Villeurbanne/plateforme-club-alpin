...
if (count($errTab) && (!in_array($_POST['operation'], ['user_join_del', 'user_join_update_status', 'evt_user_contact', 'user_join'], true))) {
    ...
    if (!$evt['cycle_parent_evt']) {
        ...
        if (user() && 'neutre' != $monStatut) { ?>
			...
			<div id="inscription-annuler" style="display:<?php if ('user_join_del' == $_POST['operation']) {
                ...
                // TABLEAU d'erreurs
                if ('user_join_del' == $_POST['operation'] && count($errTab)) {
                    echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
                } ?>

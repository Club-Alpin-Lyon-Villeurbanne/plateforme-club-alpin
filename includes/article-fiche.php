...
if(!$article) echo '<p class="erreur">Erreur : article non trouvé ou non autorisé</p>';
else{
	...
	<div class="titleimage" style="background-image:url(<?php echo $img;?>)">
		...
		<p class="auteur">
			<?php
				...
				if($article['commission_article'] == -1 && $article['evt']){
	...
	} elseif ($article['status_article'] != '1') {
	...
	if ((allowed('article_delete_notmine') || allowed('article_delete', 'commission:'.$article['commission_article']) || allowed('article_edit_notmine') || allowed('article_edit', 'commission:'.$article['commission_article'] )) && $article['status_article']==1) {
	...
	if ($article['status_article']!='1' && (allowed('article_delete_notmine') or allowed('article_delete', 'commission:'.$article['commission_article'] ))) {
		// Suppression
	...
	} elseif (allowed('article_validate_all') or allowed('article_edit', 'commission:'.$article['commission_article'] )) {
	...
	// remonter en tete
	if ( allowed('article_validate_all') &&  ($article['status_article'] == 1)) {
...
//		echo '<input class="nice2 green" type="button" value="&nbsp;&nbsp;Remonter en tête" onclick="javascript:document.forms[\'renew-date-article-'.$article['id_article'].'\'].submit();" />';
	...
	if ($article['topubly_article'] != '1' || allowed('article_validate', 'commission:'.$article['commission_article']) || allowed('article_validate_all') || allowed('article_delete_notmine') || allowed('article_delete', 'commission:'.$article['commission_article']) || allowed('article_edit_notmine') or allowed('article_edit', 'commission:'.$article['commission_article'] )) {
		echo '</div><br />';
	}
	...
	if($article['status_article']=='1'){
		...
		<aside>
			...
			if(user())	include INCLUDES.'commenter-online.php';

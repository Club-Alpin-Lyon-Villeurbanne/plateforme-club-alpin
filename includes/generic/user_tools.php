<a id="toolbar-user" href="javascript:void(0)" title="" class="toptrigger">
	...
	// CONNECTÉ
		...
		<div class="connected-name">
			...
			if($_SESSION['user']['doit_renouveler_user'] or $_SESSION['user']['alerte_renouveler_user'])
		...
		// CONNECTÉ
			...
			<div style="width:515px; min-height:160px; float:left; border-right:1px solid #c6e39f; min-height:130px; padding:0px 5px 0 0">
				...
				<div class="nav-user">
                    ...
                    if(allowed('destination_creer') OR allowed('destination_modifier'))
				...
				if(			allowed('user_see_all')
						or 	allowed('user_create_manually')
						or 	allowed('user_updatefiles')
						){
					?>
					<div class="nav-user">
...
/* Supprimer également l'entrée de caf-page avec la clé fichier-adherents */
				...
				if(		allowed('comm_create')
					or 	allowed('comm_edit')
					or 	allowed('comm_desactivate')
					or 	allowed('comm_delete')
					){
					?>
					<div class="nav-user">
			...
			<div style="width:340px; float:right; padding:0px 10px 0 0; ">
				...
				if(sizeof($_SESSION['user']['status'])){
					...
					if(preg_match("#Encadrant|Resp\. de commission|Rédacteur#", $_SESSION['user']['status'][0])){
						// statut...
						$statut=strtolower(substr(strchr($_SESSION['user']['status'][0], '.'), 0));
						...
						if(in_array($statut.', '.$p2, $_SESSION['user']['status'])){
							if(($key = array_search($statut.', '.$p2, $_SESSION['user']['status'])) !== false) {
					...
					if(sizeof($_SESSION['user']['status'])>1) echo ' <a href="profil.html" title="Entre autres choses..." style="color:white; font-weight:100; font-family:Arial">[+]</a>';
					echo '</p>';
				}
				else echo '<p class="status">Vous êtes connecté en tant qu\'<span style="font-family:DINBold">adhérent</span></p>';
				?>
			</div>

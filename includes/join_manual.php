	...
	elseif(!$id_evt){
		...
		if(!is_array($_POST['id_user'])){
			...
			<p>
				...
				<br />
				<a href="<?php echo $versCettePage;?>" <?php if(!$showAll) echo 'style="background:#d3d6ff"';?> class="boutonFancy">Voir uniquement les adhérents validés</a>
				<a href="<?php echo $versCettePage;?>&showAll=1" <?php if($showAll) echo 'style="background:#d3d6ff"';?> class="boutonFancy">Voir tous les adhérents de la base (+long)</a>
					...
					<tbody>
						...
						while($elt = $result->fetch_assoc()){
                            ...
                            if ($id_dest) {
                                ...
                                $req="SELECT COUNT(id_evt_join) FROM caf_evt_join WHERE (id_destination=$id_dest OR evt_evt_join IN (".implode(',', $sorties_ids).")) AND user_evt_join = ".intval($elt['id_user'])." LIMIT 1";

                            }
                            //sinon
                            else {
                                // SELECTION : on n'affiche que les adhérents qui ne sont pas inscrit à cette sortie
                                $req="SELECT COUNT(id_evt_join) FROM caf_evt_join WHERE evt_evt_join=$id_evt AND user_evt_join = ".intval($elt['id_user'])." LIMIT 1";
							...
							if(!$row[0]){
								...
								echo '<tr id="tr-'.$elt['id_user'].'" class="'.($elt['valid_user']?'vis-on':'vis-off').'">'
									...
									.'<td>'
										...
										.'<input type="checkbox" name="id_user[]" value="'.intval($elt['id_user']).'" />'
		...
		// ENSUITE, CONFIRMATION ET ENVOI :
			...
			if(!sizeof($_POST['id_user'])){
				...
				<form action="<?php echo $versCettePage;?>" method="post" enctype="multipart/form-data" class="loading">
					...
					if($_POST['operation'] == 'user_join_manuel' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
					// redirection en cas de réussite
					if($_POST['operation'] == 'user_join_manuel' && !sizeof($errTab)){
						?>
						...
						<tbody>
							...
							foreach ($_POST['id_user'] as $i => $utilisateur){
								...
								echo '<tr>'
                                    .'<td>'
                                        // vars to re-post
                                        .'<input type="hidden" name="id_user[]" value="'.intval($_POST['id_user'][$i]).'" />'
                                    ...
                                    .'<td>'
                                        .'<select name="role_evt_join[]">'
                                            ...
                                            .($sortie['need_benevoles_evt']==1?'<option value="benevole">Bénévole</option>':'')
                                    ...
                                    .($sortie['repas_restaurant'] == '1' ? '<td>'
                                        .'<select name="is_restaurant[]">'
                                            ...
                                            .'<option value="0" '.($_POST['is_restaurant'][$i] == '0' ? ' selected="selected" ' : '').'>Non</option>'
                                            .'<option value="1" '.($_POST['is_restaurant'][$i] == '1' ? ' selected="selected" ' : '').'>Oui</option>'
                                        .'</select>'
                                    .'</td>' : '');
                                    if ($id_dest) { echo '<td>'
                                        .'<select name="id_bus_lieu_destination[]">'
                                        .'<option value="-1" '.($_POST['id_bus_lieu_destination'][$i] == '-1' ? ' selected="selected" ' : '').'>Covoiturage</option>';
                                        ...
                                        ?>
                                        <option value="<?php echo $point['bdl_id']; ?>" <?php if ($_POST['id_bus_lieu_destination'][$i] == $point['bdl_id']) echo  ' selected="selected" '; ?>>

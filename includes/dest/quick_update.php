...
if (
    ...
    if (
        ...
        echo
            ...
            '<form action="'.$versCettePage.'" method="post" style="display:inline" class="loading">
                ...
                <input type="hidden" name="id_dest_to_update" value="'.(intval($destination['id'])).'" />
    ...
    if (
        ...
        echo
            (intval($destination['annule']) == 0
            ?   (date('Y-m-d H:i:s') > $destination['inscription_fin'] ? '' : (intval($destination['inscription_locked']) == 0
                ?   '<form action="'.$versCettePage.'" method="post" style="display:inline" class="loading">
    ...
    if (
        ...
        echo
            (intval($destination['annule']) == 0
                ?   '<a class="nice2 noprint red" href="annuler-une-sortie/destination/'.$destination['code'].'-'.intval($destination['id']).'.html" title="Annuler la destination ci-dessous et toutes les sorties">

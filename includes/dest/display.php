...
// CONTENU LIBRE
    ...
    <?php } else { ?>
        <ul class="nice-list">
            <li class="wide">Ouverture : <?php echo display_jour($destination['inscription_ouverture']); if(allowed('user_read_limited')){ echo ', à '. display_time($destination['inscription_ouverture']); } ?></li>
            <li class="wide">Fermeture : <?php echo display_jour($destination['inscription_fin']); if(allowed('user_read_limited')){ echo ', à '. display_time($destination['inscription_fin']); } ?></li>
        </ul>
    <?php } ?>
    <hr class="clear">
                ...
                if ($destination['cout_transport'] > 0) {
                    echo '<ul class="nice-list">'
                        ...
                        .($destination['cout_transport'] > 0
                            ? '<li class="wide"><b>COÛT :</b> '.str_replace(',', '.', floatVal($destination['cout_transport'])).'&nbsp;Euros</li>'
                            : '').'</ul><br>';
                }
            ?>
    ...
    if(allowed('user_read_limited')){ echo ', '.display_time($destination['date']);}
    echo '</li>';

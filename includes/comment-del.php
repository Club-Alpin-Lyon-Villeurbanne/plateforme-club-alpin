...
if(!$id_comment) echo "<p class='erreur'>ID commentaire introuvable.</p>";
else{
	...
	if(!$comment) echo "<p class='erreur'>Commentaire introuvable.</p>";
	...
	elseif($comment['status_comment']==1){
		...
		<form action="<?php echo $versCettePage;?>" method="post" id="comment-form">
			...
			<?php
			// mon commentaire
			if($comment['user_comment'] == $_SESSION['user']['id_user']) inclure('infos-supprimer-mon-commentaire');
			// ce commentaire (droit special)
			else inclure('infos-supprimer-any-commentaire');

			// MESSAGES A LA SOUMISSION
			if($_POST['operation'] == 'comment_hide' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
			?>
			<br />

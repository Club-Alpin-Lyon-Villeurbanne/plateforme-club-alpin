...
<div id="trigger-userinfo" style="display:<?php if($_POST['operation']!='user_contact') echo 'none'; ?>">
    ...
    <form action="<?php echo $versCettePage;?>" method="post">
        ...
        <input type="hidden" name="id_user" value="<?php echo intval($tmpUser['id_user']);?>" />

        <h2>Formulaire de contact</h2>
        <?php
        // MESSAGES A LA SOUMISSION
        if($_POST['operation'] == 'user_contact' && sizeof($errTab))	echo '<div class="erreur">Erreur : <ul><li>'.implode('</li><li>', $errTab).'</li></ul></div>';
        if($_POST['operation'] == 'user_contact' && !sizeof($errTab)) {

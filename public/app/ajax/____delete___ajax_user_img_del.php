...
if (user()) {
    ...
    $id_img = (int) ($_POST['id_img']); // facultatif
    $fichier_img = formater($_POST['fichier_img'], 4); // sécurité, empècher les ../

    if (!$fichier_img) {
        $errTab[] = 'Erreur fichier_img vide';
    }
    if (!$id_user) {
        $errTab[] = 'Erreur id_user vide';
    }
    ...
    if ((!isset($errTab) || 0 === count($errTab)) && $id_img) {
        ...
        if (!$mysqli->query($req)) {
            $errTab[] = "Erreur SQL lors de la suppression (img=$id_img et user=$id_user)";
        }
        $mysqli->close();
    }
    ...
    if (!isset($errTab) || 0 === count($errTab)) {
        ...
        // min
        if (is_file($dossier.'/'.'min-'.$fichier_img)) {
            unlink($dossier.'/'.'min-'.$fichier_img);
        }
        // pic
        if (is_file($dossier.'/'.'pic-'.$fichier_img)) {
            unlink($dossier.'/'.'pic-'.$fichier_img);
        }
    }
} else {
    $errTab[] = 'not logged';
